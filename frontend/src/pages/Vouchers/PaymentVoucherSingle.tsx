                                                    ...
                                                    <tr key={index} className={`transition-colors ${isProblemRow ? 'bg-red-50/30' : 'hover:bg-gray-50'}`}>
                                                        ...
                                                        <td className="px-6 py-4 text-right">
                                                            <NumericFormat
                                                                thousandSeparator="," thousandsGroupStyle="lakh" decimalScale={2} fixedDecimalScale={true} allowNegative={false}
                                                                value={txn.payment || ''}
                                                                onValueChange={(values) => handlePaymentChange(index, Math.max(0, parseFloat(values.value) || 0))}
                                                                placeholder="0"
                                    ...
                                    <div className="space-y-3">
                                        {paymentRows.map((row) => (
                                            <NumericFormat
                                                ...
                                                thousandSeparator="," thousandsGroupStyle="lakh" decimalScale={2} fixedDecimalScale={true} allowNegative={false}
                                                value={row.amount || ''}
                                                onValueChange={values => handlePaymentRowChange(row.id, 'amount', Math.max(0, parseFloat(values.value) || 0))}
