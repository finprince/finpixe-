...
const VendorViewModal: React.FC<VendorViewModalProps> = ({ vendorId, onClose }) => {
            ...
            {banking.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {banking.map((bank: any, idx: number) => (
                        <div key={idx} className="relative overflow-hidden bg-gradient-to-br from-[#6366F1] to-[#4338CA] rounded-[2rem] p-8 shadow-2xl group border border-white/5">
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between gap-12">
                                <div className="flex justify-between items-start">
                                    <div className="flex flex-col gap-0.5">
                                        <span className="text-indigo-200 text-[10px] font-black uppercase tracking-widest">{bank.account_type || 'Current Account'}</span>
                                ...
                                <div>
                                    <span className="text-indigo-200 text-[10px] font-bold uppercase tracking-widest block mb-2 opacity-60">Account Number</span>
    ...
    const renderTermsInfo = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="md:col-span-2 section-card bg-[#4F46E5] rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-8 text-white shadow-xl">
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 lg:p-10">
            <div className="bg-[#FAF9F6] w-full h-full max-w-7xl rounded-3xl overflow-hidden shadow-2xl flex flex-col border border-white/20 animate-in zoom-in-95 duration-300">
                ...
                <header className="bg-white px-8 pt-8 pb-4 border-b border-slate-200 shadow-sm relative overflow-hidden">
                  ...
                  ${activeTab === tab.id
                                        ? 'bg-[#6366F1] text-white shadow-lg shadow-indigo-100 -translate-y-0.5'
                                        : 'text-[#818CF8] hover:text-[#4F46E5] hover:bg-slate-100'
                                    }`}
                            >
                                <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-white' : 'text-[#C7D2FE]'}`} />
                                {tab.label}
                            </button>
                        ))}
                    </nav>
                ...
                <footer className="bg-white px-8 py-6 border-t border-slate-200 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            className="flex items-center gap-2 px-6 py-3 bg-[#6366F1] text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-[#4F46E5] transition-all shadow-md active:scale-95"
