...
import json
from .database import CustomerMaster, CustomerMasterCategory, CustomerMastersSalesQuotation, CustomerMastersSalesOrder, CustomerMasterCustomer, CustomerTransaction, CustomerMasterLongTermContractBasicDetail, CustomerMasterLongTermContractProductService, CustomerMasterLongTermContractTermsCondition, CustomerTransactionSalesQuotationGeneral, CustomerTransactionSalesQuotationSpecific, CustomerTransactionSalesOrderBasicDetails, CustomerTransactionSalesOrderItemDetails, CustomerTransactionSalesOrderDeliveryTerms, CustomerTransactionSalesOrderPaymentAndSalesperson, CustomerTransactionSalesOrderQuotationDetails
...
class CustomerMasterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'customer_code', 'customer_name', 'email', 'phone', 'mobile', 'address_line1', 'address_line2', 'city', 'state', 'country', 'pincode', 'gstin', 'pan', 'category', 'credit_limit', 'credit_days', 'opening_balance', 'current_balance', 'is_active', 'is_deleted', 'created_at', 'updated_at', 'created_by']
...
class CustomerMastersSalesQuotationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'series_name', 'customer_category', 'prefix', 'suffix', 'required_digits', 'current_number', 'auto_year', 'is_active', 'is_deleted', 'created_at', 'updated_at', 'created_by']
...
class CustomerMasterCustomerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'customer_name', 'customer_code', 'customer_category', 'pan_number', 'contact_person', 'email_address', 'contact_number', 'billing_currency', 'is_also_vendor', 'gst_tds_applicable', 'tax_type', 'gst_details', 'products_services', 'banking_info', 'msme_no', 'fssai_no', 'iec_code', 'eou_status', 'tcs_section', 'tcs_enabled', 'tds_section', 'tds_enabled', 'credit_period', 'credit_terms', 'penalty_terms', 'delivery_terms', 'warranty_details', 'force_majeure', 'dispute_terms', 'is_active', 'is_deleted', 'created_at', 'updated_at', 'created_by', 'updated_by']
        read_only_fields = ['id', 'tenant_id', 'created_by', 'created_at', 'updated_at']
    ...
    def validate_customer_code(self, value):
        ...
        if queryset.exists():
            raise serializers.ValidationError(f"Customer with code '{value}' already exists. Please use a different customer code.")
        return value
    ...
    def to_representation(self, instance):
        ...
        for gst in instance.gst_details.all():
            ...
            processed_branch = {'id': gst.id, 'gstin': gst.gstin, 'defaultRef': gst.branch_reference_name, 'addressLine1': addressLine1, 'addressLine2': addressLine2, 'addressLine3': addressLine3, 'city': city, 'pincode': pincode, 'state': state, 'country': country, 'contactPerson': gst.branch_contact_person, 'email': gst.branch_email, 'contactNumber': gst.branch_contact_number}
            processed_branches.append(processed_branch)
            dropdown_branches.append({'id': gst.id, 'gstin': gst.gstin, 'branch_reference_name': gst.branch_reference_name or 'Main Branch', 'address_line_1': addressLine1, 'address_line_2': addressLine2, 'address_line_3': addressLine3, 'city': city, 'state': state, 'pincode': pincode, 'country': country, 'addressLine1': addressLine1, 'addressLine2': addressLine2, 'addressLine3': addressLine3, 'contactPerson': gst.branch_contact_person, 'email': gst.branch_email, 'contactNumber': gst.branch_contact_number})
        return {'id': instance.id, 'tenant_id': instance.tenant_id, 'customer_name': instance.customer_name, 'customer_code': instance.customer_code, 'customer_category': instance.customer_category_id, 'customer_category_name': str(instance.customer_category) if instance.customer_category else None, 'pan_number': instance.pan_number, 'contact_person': instance.contact_person, 'email_address': instance.email_address, 'contact_number': instance.contact_number, 'billing_currency': instance.billing_currency, 'is_also_vendor': instance.is_also_vendor, 'gst_tds_applicable': instance.gst_tds_applicable, 'ledger_id': instance.ledger_id, 'is_active': instance.is_active, 'is_deleted': instance.is_deleted, 'created_at': instance.created_at.isoformat() if instance.created_at else None, 'updated_at': instance.updated_at.isoformat() if instance.updated_at else None, 'created_by': instance.created_by, 'updated_by': instance.updated_by, 'gst_details': {'gstins': [gst.gstin for gst in instance.gst_details.all() if gst.gstin], 'branches': processed_branches}, 'branches': dropdown_branches, 'products_services': {'items': [{'id': item.id, 'itemCode': item.item_code, 'itemName': item.item_name, 'hsnCode': item.hsn_code, 'uom': item.uom, 'custItemCode': item.customer_item_code, 'custItemName': item.customer_item_name, 'packingNotes': item.packing_notes} for item in instance.product_services.all()]}, 'banking_info': {'accounts': [{'id': bank.id, 'accountNumber': bank.account_number, 'bankName': bank.bank_name, 'ifscCode': bank.ifsc_code, 'branchName': bank.branch_name, 'swiftCode': bank.swift_code, 'associatedBranches': bank.associated_branches or []} for bank in instance.banking_details.all()]}, 'msme_no': getattr(tds_details, 'msme_no', None) if tds_details else None, 'fssai_no': getattr(tds_details, 'fssai_no', None) if tds_details else None, 'iec_code': getattr(tds_details, 'iec_code', None) if tds_details else None, 'eou_status': getattr(tds_details, 'eou_status', None) if tds_details else None, 'tcs_section': getattr(tds_details, 'tcs_section', None) if tds_details else None, 'tcs_enabled': getattr(tds_details, 'tcs_enabled', False) if tds_details else False, 'tds_section': getattr(tds_details, 'tds_section', None) if tds_details else None, 'tds_enabled': getattr(tds_details, 'tds_enabled', False) if tds_details else False, 'tax_type': 'TCS' if tds_details and getattr(tds_details, 'tcs_section', None) else 'TDS' if tds_details and getattr(tds_details, 'tds_section', None) else 'NONE', 'credit_period': getattr(terms_conditions, 'credit_period', None) if terms_conditions else None, 'credit_terms': getattr(terms_conditions, 'credit_terms', None) if terms_conditions else None, 'penalty_terms': getattr(terms_conditions, 'penalty_terms', None) if terms_conditions else None, 'delivery_terms': getattr(terms_conditions, 'delivery_terms', None) if terms_conditions else None, 'warranty_details': getattr(terms_conditions, 'warranty_details', None) if terms_conditions else None, 'force_majeure': getattr(terms_conditions, 'force_majeure', None) if terms_conditions else None, 'dispute_terms': getattr(terms_conditions, 'dispute_terms', None) if terms_conditions else None}
    ...
    def create(self, validated_data):
        ...
        """
        from .models import CustomerMasterCustomerGSTDetails, CustomerMasterCustomerProductService, CustomerMasterCustomerTDS, CustomerMasterCustomerBanking, CustomerMasterCustomerTermsCondition
        ...
        tds_data = {'msme_no': validated_data.pop('msme_no', None), 'fssai_no': validated_data.pop('fssai_no', None), 'iec_code': validated_data.pop('iec_code', None), 'eou_status': validated_data.pop('eou_status', None), 'tcs_section': validated_data.pop('tcs_section', None), 'tcs_enabled': validated_data.pop('tcs_enabled', False), 'tds_section': validated_data.pop('tds_section', None), 'tds_enabled': validated_data.pop('tds_enabled', False)}
        logger.info(f'TDS Data: {tds_data}')
        terms_data = {'credit_period': validated_data.pop('credit_period', None), 'credit_terms': validated_data.pop('credit_terms', None), 'penalty_terms': validated_data.pop('penalty_terms', None), 'delivery_terms': validated_data.pop('delivery_terms', None), 'warranty_details': validated_data.pop('warranty_details', None), 'force_majeure': validated_data.pop('force_majeure', None), 'dispute_terms': validated_data.pop('dispute_terms', None)}
        ...
        try:
            with transaction.atomic():
                ...
                for branch in branches:
                    ...
                    address_data = {'addressLine1': branch.get('addressLine1', branch.get('address', '')), 'addressLine2': branch.get('addressLine2', ''), 'addressLine3': branch.get('addressLine3', ''), 'city': branch.get('city', ''), 'pincode': branch.get('pincode', ''), 'state': branch.get('state', ''), 'country': branch.get('country', 'India')}
                    branch_address_json = json.dumps(address_data)
                    CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, gstin=branch_gstin, branch_reference_name=branch.get('defaultRef'), branch_address=branch_address_json, address_line_1=branch.get('addressLine1', branch.get('address', '')), address_line_2=branch.get('addressLine2', ''), address_line_3=branch.get('addressLine3', ''), city=branch.get('city', ''), state=branch.get('state', ''), country=branch.get('country', 'India'), pincode=branch.get('pincode', ''), branch_contact_person=branch.get('contactPerson'), branch_email=branch.get('email'), branch_contact_number=branch.get('contactNumber'), created_by=basic_details.created_by)
                    if branch_gstin:
                        processed_gstins.add(branch_gstin)
                for gstin in gstins:
                    if gstin and gstin not in processed_gstins:
                        CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, gstin=gstin, is_unregistered=False, created_by=basic_details.created_by)
                        processed_gstins.add(gstin)
                if not processed_gstins and (not branches):
                    CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, gstin=None, is_unregistered=True, created_by=basic_details.created_by)
                ...
                for item in items:
                    ...
                    prod_record = CustomerMasterCustomerProductService.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, item_code=item_code, item_name=item.get('itemName'), hsn_code=item.get('hsnCode'), uom=item.get('uom'), customer_item_code=item.get('custItemCode'), customer_item_name=item.get('custItemName'), packing_notes=item.get('packingNotes'), created_by=basic_details.created_by)
                ...
                if created_count == 0:
                    ...
                    prod_record = CustomerMasterCustomerProductService.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, item_code=None, item_name=None, created_by=basic_details.created_by)
                ...
                if created_count == 0:
                    ...
                    bank_record = CustomerMasterCustomerBanking.objects.create(customer_basic_detail=basic_details, tenant_id=basic_details.tenant_id, account_number=None, bank_name=None, ifsc_code=None, created_by=basic_details.created_by)
        ...
        with transaction.atomic():
            ...
            if gst_details_data is not None:
                ...
                for branch in branches:
                    ...
                    address_data = {'addressLine1': branch.get('addressLine1', branch.get('address', '')), 'addressLine2': branch.get('addressLine2', ''), 'addressLine3': branch.get('addressLine3', ''), 'city': branch.get('city', ''), 'pincode': branch.get('pincode', ''), 'state': branch.get('state', ''), 'country': branch.get('country', 'India')}
                    branch_address_json = json.dumps(address_data)
                    CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=instance, tenant_id=instance.tenant_id, gstin=branch_gstin, branch_reference_name=branch.get('defaultRef'), branch_address=branch_address_json, address_line_1=branch.get('addressLine1', branch.get('address', '')), address_line_2=branch.get('addressLine2', ''), address_line_3=branch.get('addressLine3', ''), city=branch.get('city', ''), state=branch.get('state', ''), country=branch.get('country', 'India'), pincode=branch.get('pincode', ''), branch_contact_person=branch.get('contactPerson'), branch_email=branch.get('email'), branch_contact_number=branch.get('contactNumber'), updated_by=instance.updated_by)
                    if branch_gstin:
                        processed_gstins.add(branch_gstin)
                for gstin in gstins:
                    if gstin and gstin not in processed_gstins:
                        CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=instance, tenant_id=instance.tenant_id, gstin=gstin, is_unregistered=False, updated_by=instance.updated_by)
                        processed_gstins.add(gstin)
                if not processed_gstins and (not branches):
                    CustomerMasterCustomerGSTDetails.objects.create(customer_basic_detail=instance, tenant_id=instance.tenant_id, gstin=None, is_unregistered=True, updated_by=instance.updated_by)
            if products_services_data is not None:
                ...
                for item in items:
                    CustomerMasterCustomerProductService.objects.create(customer_basic_detail=instance, tenant_id=instance.tenant_id, item_code=item.get('itemCode'), item_name=item.get('itemName'), hsn_code=item.get('hsnCode'), uom=item.get('uom'), customer_item_code=item.get('custItemCode'), customer_item_name=item.get('custItemName'), packing_notes=item.get('packingNotes'), updated_by=instance.updated_by)
            if any(tds_data.values()):
                tds_instance, created = CustomerMasterCustomerTDS.objects.get_or_create(customer_basic_detail=instance, defaults={'tenant_id': instance.tenant_id})
            ...
            if banking_info_data is not None:
                ...
                for account in accounts:
                    if account.get('accountNumber'):
                        CustomerMasterCustomerBanking.objects.create(customer_basic_detail=instance, tenant_id=instance.tenant_id, account_number=account.get('accountNumber'), bank_name=account.get('bankName'), ifsc_code=account.get('ifscCode'), branch_name=account.get('branchName'), swift_code=account.get('swiftCode'), associated_branches=account.get('associatedBranches'), updated_by=instance.updated_by)
            if any(terms_data.values()):
                terms_instance, created = CustomerMasterCustomerTermsCondition.objects.get_or_create(customer_basic_detail=instance, defaults={'tenant_id': instance.tenant_id})
...
class CustomerTransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'customer', 'customer_id', 'transaction_type', 'voucher_number', 'transaction_number', 'date', 'transaction_date', 'amount', 'tax_amount', 'total_amount', 'debit', 'credit', 'payment_status', 'payment_mode', 'reference_number', 'notes', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class CustomerMasterLongTermContractProductServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'contract_basic_detail', 'item_code', 'item_name', 'customer_item_name', 'uom', 'qty_min', 'qty_max', 'price_min', 'price_max', 'acceptable_price_deviation', 'created_at', 'updated_at', 'created_by']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class CustomerMasterLongTermContractTermsConditionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'contract_basic_detail', 'payment_terms', 'penalty_terms', 'force_majeure', 'termination_clause', 'dispute_terms', 'others', 'created_at', 'updated_at', 'created_by']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class CustomerTransactionSalesOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'tenant_id', 'so_series_name', 'so_number', 'date', 'customer_po_number', 'customer_name', 'gst_no', 'branch', 'address', 'email', 'contact_number', 'status', 'is_active', 'is_deleted', 'created_at', 'updated_at', 'created_by', 'updated_by', 'items', 'delivery_terms', 'payment_and_salesperson', 'quotation_details']
    ...
    def to_representation(self, instance):
        ...
        if not repr.get('address'):
            try:
                ...
                customer = CustomerMasterCustomerBasicDetails.objects.filter(tenant_id=instance.tenant_id, customer_name=instance.customer_name).first()
                if customer:
                    branch_qs = CustomerMasterCustomerGSTDetails.objects.filter(customer_basic_detail=customer)
                    branch_match = None
                    if instance.branch:
        ...
        try:
            with transaction.atomic():
                ...
                for item_data in items_data:
                    ...
                    CustomerTransactionSalesOrderItemDetails.objects.create(so_basic_detail=sales_order, **item_data)
                logger.info(f'[OK] Created {len(items_data)} items')
                if delivery_terms_data:
                    delivery_terms_data['tenant_id'] = sales_order.tenant_id
                    CustomerTransactionSalesOrderDeliveryTerms.objects.create(so_basic_detail=sales_order, **delivery_terms_data)
                ...
                if payment_and_salesperson_data:
                    ...
                    CustomerTransactionSalesOrderPaymentAndSalesperson.objects.create(so_basic_detail=sales_order, **payment_and_salesperson_data)
                ...
                if quotation_details_data:
                    ...
                    CustomerTransactionSalesOrderQuotationDetails.objects.create(so_basic_detail=sales_order, **quotation_details_data)
            ...
            with transaction.atomic():
                ...
                if items_data is not None:
                    ...
                    for item_data in items_data:
                        ...
                        CustomerTransactionSalesOrderItemDetails.objects.create(so_basic_detail=instance, **item_data)
                ...
                if delivery_terms_data is not None:
                    ...
                    CustomerTransactionSalesOrderDeliveryTerms.objects.create(so_basic_detail=instance, **delivery_terms_data)
                ...
                if payment_and_salesperson_data is not None:
                    ...
                    CustomerTransactionSalesOrderPaymentAndSalesperson.objects.create(so_basic_detail=instance, **payment_and_salesperson_data)
                ...
                if quotation_details_data is not None:
                    ...
                    CustomerTransactionSalesOrderQuotationDetails.objects.create(so_basic_detail=instance, **quotation_details_data)
