...
def mirror_purchase_to_portal(purchase_header):
    ...
    try:
        ...
        vendor = VendorMasterBasicDetail.objects.filter(tenant_id=tenant_id, id=purchase_header.vendor_basic_detail_id).first()
        if not vendor:
            vendor = VendorMasterBasicDetail.objects.filter(tenant_id=tenant_id, vendor_name__iexact=str(purchase_header.vendor_name).strip()).first()
        if not vendor:
            return
        ...
        VendorTransaction.objects.update_or_create(tenant_id=tenant_id, vendor_id=vendor.id, transaction_number=v_num, transaction_type='purchase', defaults={'transaction_date': purchase_header.date, 'amount': total_amt, 'total_amount': total_amt, 'status': tx_status, 'reference_number': v_num, 'reference_type': 'invoice', 'is_advance': False, 'notes': getattr(purchase_header, 'voucher_name', None) or 'Purchase Invoice', 'ledger_name': vendor.vendor_name or 'Vendor'})
...
def mirror_sales_to_portal(sales_header):
    ...
    try:
        ...
        customer = CustomerMasterCustomer.objects.filter(tenant_id=tenant_id, id=sales_header.customer_id).first()
        if not customer and sales_header.customer_name:
            customer = CustomerMasterCustomer.objects.filter(tenant_id=tenant_id, customer_name__iexact=str(sales_header.customer_name).strip()).first()
        if not customer:
            return
        payment_details = sales_header.payment_details if hasattr(sales_header, 'payment_details') else None
        total_amt = Decimal(str(payment_details.payment_invoice_value if payment_details else 0))
        CustomerTransaction.objects.update_or_create(tenant_id=tenant_id, transaction_number=sales_header.sales_invoice_no, transaction_type='invoice', customer_id=customer.id, defaults={'transaction_date': sales_header.date, 'total_amount': total_amt, 'amount': total_amt, 'payment_status': sales_header.status or 'Unpaid', 'reference_number': sales_header.sales_invoice_no, 'notes': sales_header.voucher_name or 'Sales Invoice'})
        if payment_details and payment_details.payment_advance and (float(payment_details.payment_advance) > 0):
            CustomerTransaction.objects.filter(tenant_id=tenant_id, customer_id=customer.id, transaction_type='RECEIPT', transaction_number__contains=f'-ADJ{sales_header.id}I').delete()
            ...
            if adv_refs and isinstance(adv_refs, list):
                for idx, ref in enumerate(adv_refs):
                    ...
                    if is_selected and float(allocated_amt) > 0:
                        ...
                        CustomerTransaction.objects.update_or_create(tenant_id=tenant_id, customer_id=customer.id, transaction_number=display_ref, transaction_type='RECEIPT', defaults={'transaction_date': sales_header.date, 'amount': Decimal(str(allocated_amt)), 'total_amount': Decimal(str(allocated_amt)), 'payment_status': 'Advance Applied', 'reference_number': sales_header.sales_invoice_no, 'notes': f'Advance adjusted from Ref: {ref_no}'})
            else:
                CustomerTransaction.objects.update_or_create(tenant_id=tenant_id, customer_id=customer.id, transaction_number=f'ADJ-{sales_header.sales_invoice_no}', transaction_type='RECEIPT', defaults={'transaction_date': sales_header.date, 'amount': payment_details.payment_advance, 'total_amount': payment_details.payment_advance, 'payment_status': 'Advance Applied', 'reference_number': sales_header.sales_invoice_no, 'notes': 'Advance adjusted'})
...
def _mirror_payment_item(voucher, item, vendor):
    ...
    VendorTransaction.objects.update_or_create(tenant_id=voucher.tenant_id, vendor_id=vendor.id, transaction_number=f'{voucher.voucher_number}-{item.id}', transaction_type='payment', defaults={'transaction_date': voucher.date, 'amount': item.allocated_amount or item.amount, 'total_amount': item.allocated_amount or item.amount, 'status': status, 'reference_number': ref, 'reference_type': getattr(item, 'reference_type', 'invoice'), 'is_advance': is_adv, 'notes': f'Payment for {ref}' if ref and ref != 'ADVANCE' else voucher.narration or 'Payment', 'ledger_name': vendor.vendor_name or 'Vendor'})
...
def _mirror_receipt_item(voucher, item, customer):
    ...
    ref = item.reference_number or item.advance_ref_no or voucher.voucher_number
    CustomerTransaction.objects.update_or_create(tenant_id=voucher.tenant_id, customer_id=customer.id, transaction_number=f'{voucher.voucher_number}-{item.id}', transaction_type='receipt', defaults={'transaction_date': voucher.date, 'amount': item.allocated_amount or item.amount or item.received_amount, 'total_amount': item.allocated_amount or item.amount or item.received_amount, 'payment_status': status, 'reference_number': ref, 'reference_type': getattr(item, 'reference_type', 'invoice'), 'is_advance': is_adv, 'notes': voucher.narration or f'Receipt for {ref}', 'ledger_name': 'Receipt'})
...
def _update_portal_sales_status(tenant_id, customer_id, ref_no):
    ...
    try:
        invoice = CustomerTransaction.objects.filter(tenant_id=tenant_id, customer_id=customer_id, transaction_type='invoice', reference_number=ref_no).first()
        if invoice:
            total_amt = Decimal(str(invoice.total_amount or 0))
            if total_amt <= 0:
                return
            received = CustomerTransaction.objects.filter(tenant_id=tenant_id, customer_id=customer_id, transaction_type__in=['receipt', 'credit_note'], reference_number=ref_no).aggregate(s=models.Sum('total_amount'))['s'] or 0
            debits = CustomerTransaction.objects.filter(tenant_id=tenant_id, customer_id=customer_id, transaction_type__in=['debit_note'], reference_number=ref_no).aggregate(s=models.Sum('total_amount'))['s'] or 0
            ...
            try:
                ...
                if terms and terms.credit_period:
                    match = re.search('(\\d+)', str(terms.credit_period))
                    if match:
...
def _update_portal_purchase_status(tenant_id, vendor_id, ref_no):
    ...
    try:
        purchase = VendorTransaction.objects.filter(tenant_id=tenant_id, vendor_id=vendor_id, transaction_type='purchase', reference_number=ref_no).first()
        if purchase:
            total_amt = Decimal(str(purchase.total_amount or 0))
            if total_amt <= 0:
                return
            paid = VendorTransaction.objects.filter(tenant_id=tenant_id, vendor_id=vendor_id, transaction_type='payment', reference_number=ref_no).aggregate(s=models.Sum('total_amount'))['s'] or 0
...
def sync_portal_allocation_to_main_ledger(portal_instance):
    ...
    try:
        ...
        invoice_obj = VoucherSalesInvoiceDetails.objects.filter(tenant_id=portal_instance.tenant_id, sales_invoice_no=ref_no).first()
...
def delete_purchase_from_portal(purchase_header):
    try:
        VendorTransaction.objects.filter(tenant_id=purchase_header.tenant_id, transaction_number=purchase_header.purchase_voucher_no or purchase_header.supplier_invoice_no, transaction_type='purchase').delete()
    except Exception as e:
        logger.error(f'Failed to delete purchase mirror: {e}')

def delete_sales_from_portal(sales_header):
    try:
        CustomerTransaction.objects.filter(tenant_id=sales_header.tenant_id, transaction_number=sales_header.sales_invoice_no, transaction_type='invoice').delete()
    except Exception as e:
        logger.error(f'Failed to delete sales mirror: {e}')

def delete_transaction_from_portal(txn):
    try:
        VendorTransaction.objects.filter(tenant_id=txn.tenant_id, transaction_number__startswith=f'{txn.voucher_number}-').delete()
        CustomerTransaction.objects.filter(tenant_id=txn.tenant_id, transaction_number__startswith=f'{txn.voucher_number}-').delete()
    except Exception as e:
