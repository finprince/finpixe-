...
class GSTR2ViewSet(viewsets.ViewSet):
    ...
    def b2b(self, request):
        ...
        for v in vouchers:
            ...
            # Default to Intrastate/Interstate logic
            pos = ''
            if v.gstin and len(v.gstin) >= 2:
                pos = v.gstin[:2]
            elif v.input_type == 'Intrastate': pos = '29' 
            elif v.input_type == 'Interstate': pos = '27'
            ...
            data.append({
                ...
                'invoice_date': v.supplier_invoice_date or v.date,
                'invoice_value': val,
                'place_of_supply': pos,
                'reverse_charge': 'N',
    ...
    def impg(self, request):
        ...
        for v in vouchers:
            ...
            data.append({
                ...
                'supplier_name': v.vendor_name,
                'port_code': '',
                'boe_no': v.supplier_invoice_no or v.purchase_voucher_no,
                'boe_date': v.supplier_invoice_date or v.date,
                'boe_value': val,
                'taxable_value': taxable,
                'igst': igst,
                'cess': sum([item.cess_amount for item in items]),
                'source': 'impg_drilldown'
            })
        return Response(data)
    ...
    def stats(self, request):
        ...
        return Response({
            'B2B': b2b_qs.count(),
            'B2BUR': b2bur_qs.count(),
            'IMPG': impg_qs.count(),
            'CDNR': cdnr_qs.count(),
            'CDNUR': cdnur_qs.count(),
        })
