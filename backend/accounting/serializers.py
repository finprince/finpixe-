...
from .models import MasterLedgerGroup, MasterLedger, MasterHierarchyRaw, Voucher, JournalEntry, AmountTransaction, PaymentVoucherItem
...
class MasterLedgerSerializer(BranchModelSerializerMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = MasterLedger
        fields = ['id', 'name', 'code', 'category', 'group', 'sub_group_1', 'sub_group_2', 'sub_group_3', 'ledger_type', 'gstin', 'registration_type', 'state', 'extended_data', 'additional_data', 'question_answers', 'opening_balance', 'opening_balance_type', 'balance', 'parent_ledger_id', 'tenant_id', 'created_at', 'updated_at']
        read_only_fields = ['id', 'code', 'balance', 'tenant_id', 'created_at', 'updated_at']
        extra_kwargs = {'name': {'required': False, 'allow_null': True, 'allow_blank': True}, 'category': {'required': False, 'allow_blank': True, 'default': ''}, 'group': {'required': False, 'allow_null': True, 'allow_blank': True}, 'sub_group_1': {'required': False, 'allow_null': True, 'allow_blank': True}, 'sub_group_2': {'required': False, 'allow_null': True, 'allow_blank': True}, 'sub_group_3': {'required': False, 'allow_null': True, 'allow_blank': True}, 'gstin': {'required': False, 'allow_null': True, 'allow_blank': True}, 'registration_type': {'required': False, 'allow_null': True, 'allow_blank': True}, 'state': {'required': False, 'allow_null': True, 'allow_blank': True}, 'extended_data': {'required': False, 'allow_null': True}, 'additional_data': {'required': False, 'allow_null': True}, 'parent_ledger_id': {'required': False, 'allow_null': True}, 'opening_balance': {'required': False, 'default': 0.0}, 'opening_balance_type': {'required': False, 'allow_blank': True, 'default': 'Dr'}}
    ...
    def get_balance(self, obj):
        ...
        try:
            ...
            try:
                ...
                entries = JournalEntry.objects.filter(tenant_id=obj.tenant_id, ledger=obj)
...
class MasterHierarchyRawSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'major_group_1', 'group_1', 'sub_group_1_1', 'sub_group_2_1', 'sub_group_3_1', 'ledger_1', 'code']
...
class AmountTransactionSerializer(BranchModelSerializerMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'ledger', 'ledger_name', 'ledger_code', 'transaction_date', 'transaction_type', 'debit', 'credit', 'balance', 'voucher', 'voucher_number', 'narration', 'tenant_id', 'created_at', 'updated_at']
        read_only_fields = ['id', 'tenant_id', 'created_at', 'updated_at', 'ledger_name', 'ledger_code', 'voucher_number']
        extra_kwargs = {'debit': {'required': False, 'default': 0}, 'credit': {'required': False, 'default': 0}, 'balance': {'required': False, 'default': 0}, 'voucher': {'required': False, 'allow_null': True}, 'narration': {'required': False, 'allow_blank': True, 'allow_null': True}}
...
class JournalEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = JournalEntry
        fields = ['id', 'ledger', 'ledger_name', 'debit', 'credit', 'voucher_type', 'voucher_number', 'transaction_date', 'voucher_id', 'reference_number', 'allocation_status']
        read_only_fields = ['id']
...
class VoucherSerializer(BranchModelSerializerMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = '__all__'
        read_only_fields = ['id', 'tenant_id', 'created_at', 'updated_at']
        extra_kwargs = {'voucher_number': {'required': False}, 'party': {'required': False, 'allow_blank': True}, 'account': {'required': False, 'allow_blank': True}, 'from_account': {'required': False, 'allow_blank': True}, 'to_account': {'required': False, 'allow_blank': True}, 'ref_no': {'required': False, 'allow_blank': True}}
    ...
    def to_representation(self, instance):
        ...
        if instance.type and instance.type.lower() == 'journal':
            ...
            ret['entries'] = [{'id': e.id, 'ledger': e.ledger_id, 'ledger_name': e.ledger_name, 'debit': e.debit_amount, 'credit': e.credit_amount, 'entry_note': e.entry_note, 'reference_no': e.reference_no} for e in entries]
        if instance.type and instance.type.lower() in {'expense', 'expense voucher', 'expenses'}:
            ...
            ret['expense_rows'] = [{'id': e.id, 'expense': e.expense_ledger_name or e.expense_ledger_id, 'postTo': e.post_to_ledger_name or e.post_to_ledger_id, 'totalAmount': e.total_amount, 'billRefNo': e.bill_ref_no, 'entryNote': e.entry_note, 'expense_ledger_name': e.expense_ledger_name, 'post_to_ledger_name': e.post_to_ledger_name, 'amount': e.amount, 'total_amount': e.total_amount} for e in exp_items]
            ...
            if v_type_lower in {'receipt', 'receipts'}:
                ...
                if isinstance(instance, Transaction):
                    ...
                    if not adv and getattr(instance, 'voucher_number', None) and getattr(instance, 'tenant_id', None):
                        adv = AdvanceAllocation.objects.filter(transaction__voucher_number=instance.voucher_number, tenant_id=instance.tenant_id).first()
    ...
    def _create_journal_entries(self, voucher, validated_data, entries_data=None, tenant_id=None):
        ...
        try:
            ...
            if len(entries) >= 2:
                post_transaction(voucher_type=v_type.upper(), voucher_id=voucher.id, tenant_id=tenant_id, entries=entries, transaction_date=voucher.date, voucher_number=voucher.voucher_number)
        except Exception as e:
...
class PaymentVoucherItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'voucher', 'voucher_number', 'voucher_date', 'pay_to_ledger', 'pay_to_ledger_id', 'ledger_id', 'name', 'reference_type', 'reference_no', 'amount', 'category']
