...
def detect_pdf_type(file_bytes: bytes) -> dict:
    ...
    try:
        ...
        is_digital = usable_pages > 0 and (total_chars >= 50)

        result = {
            "pdf_type": "digital" if is_digital else "scanned",
            "text_layer_present": is_digital,
            "confidence": round(usable_pages / total_pages, 2) if total_pages > 0 else 0.0,
            "pages": total_pages,
            "char_count_total": total_chars,
            "avg_chars_per_page": round(total_chars / total_pages, 1) if total_pages > 0 else 0.0
        }
        logger.info(
