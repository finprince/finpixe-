...
class ForensicMerger:
    ...
    def __init__(self):
        self.copy_patterns = {'original': 'ORIGINAL\\s*FOR\\s*RECIPIENT', 'duplicate': 'DUPLICATE\\s*FOR\\s*TRANSPORTER', 'triplicate': 'TRIPLICATE', 'copy': '\\bCOPY\\b', 'transport_copy': 'TRANSPORT\\s*COPY'}
        self.traces = []
    ...
    def is_empty_or_zero(self, val: Any) -> bool:
        ...
        if isinstance(val, (list, dict, set, tuple)):
            return len(val) == 0
        if val in ('', 0, 0.0, '0.0', '0.00', '0.000', '—', 'N/A', 'null', 'MISSING', 'nan', 'NaN', 'None', 'undefined'):
            return True
        if isinstance(val, Decimal):
            return val == Decimal('0.00') or val.is_zero()
        if isinstance(val, str):
            val_clean = val.strip().lower()
            if val_clean in {'', 'none', '—', 'missing', 'nan', 'null', 'undefined'}:
                return True
            try:
    ...
    def safe_merge(self, primary: Dict[str, Any], continuation: Dict[str, Any]):
        ...
        """
        NUMERIC_FIELDS = {'total_taxable_value', 'taxable_value', 'subtotal', 'total_igst', 'igst', 'total_cgst', 'cgst', 'total_sgst', 'sgst', 'utgst', 'total_cess', 'cess', 'cess_amount', 'total_invoice_value', 'invoice_total', 'total_amount', 'grand_total', 'round_off', 'rounding', 'adjustment', 'rounding_adjustment'}
    ...
    def recompute_totals_if_needed(self, inv: Dict[str, Any], is_multipage: bool=False):
        ...
        if self.is_empty_or_zero(header_round_off) and ocr_text:
            ro_match = re.search('(?i)(?:Round\\s*Off|Rounding\\s*Adjustment|Adjustment|Final\\s*Adjustment)\\s*[:/-]?\\s*([+-]?[0-9]*\\.[0-9]{1,2})', ocr_text)
            if ro_match:
    ...
    def group_invoices(self, invoices: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
            ...
            if len(group_pages) > 1:
                ...
                if gstin_votes:
                    ...
                    try:
                        ...
                        db_vendor_gstins = [g.upper() for g in VendorMasterGSTDetails.objects.filter(gstin__in=candidate_gstins).values_list('gstin', flat=True)]
                    ...
                    if db_candidates:
                        ...
                        logger.info(f'[STABILIZER_GSTIN_DB_ELECTED] inv_no={inv_no} best={best_gstin} db_candidates={db_candidates} votes={dict(gstin_votes)}')
                    else:
                        best_gstin = max(gstin_votes, key=gstin_votes.get)
                        logger.info(f'[STABILIZER_GSTIN_VOTE_ELECTED] inv_no={inv_no} best={best_gstin} votes={dict(gstin_votes)}')
                    for p in group_pages:
                        p['gstin'] = best_gstin
        ...
        for i, curr in enumerate(invoices):
            ...
            if i > 0:
                ...
                prev_has_continuation = prev.get('_page_role') in {'PAGE_ROLE_PRIMARY', 'PAGE_ROLE_CONTINUATION'} or len(detect_continuation_markers(prev_ocr)) > 0
        ...
        for curr_idx, curr in enumerate(invoices):
            ...
            if role == 'PAGE_ROLE_PRIMARY' or role == 'PAGE_ROLE_UNKNOWN':
                ...
                for group_idx, group in enumerate(final_groups):
                    ...
                    logger.info(f"[GROUPING_DECISION] page_a={prev.get('_page_no')} page_b={curr.get('_page_no')} invoice_match={invoice_match} gstin_match={gstin_match} canonical_gstin_match={canonical_gstin_match} grouped={should} rejection_reason='{(reason if not should else '')}'")
                    if not should:
                        total_rejections += 1
                        page_a = prev.get('_page_no')
                        page_b = curr.get('_page_no')
                        logger.info(f"[CURRENT_DECISION] page_a={page_a} page_b={page_b} grouped=False rejection_reason='{reason}'")
                        curr_ocr = curr.get('_pdf_ocr_text') or curr.get('_raw_text') or ''
                        ...
                        if not p_no or not c_no or p_no == 'MISSING' or (c_no == 'MISSING'):
                            ...
                            from difflib import SequenceMatcher
                            p_no_clean = re.sub('[^A-Z0-9]', '', p_no)
                            c_no_clean = re.sub('[^A-Z0-9]', '', c_no)
                        ...
                        logger.info(f"[COUNTERFACTUAL_DECISION] page_a={page_a} page_b={page_b} would_group={would_group_combined} first_blocking='{first_blocking}' would_group_role_only={would_group_role_only} would_group_gstin_only={would_group_gstin_only} would_group_invoice_only={would_group_invoice_only}")
                    if should:
        ...
        for key, group in validated_groups.items():
            ...
            grouping_info = {'group_key': str(key), 'upload_session_id': str(first_page.get('upload_session_id') or ''), 'physical_file_id': str(first_page.get('record_id') or ''), 'grouped_pages': grouped_pages, 'invoice_no': str(first_page.get('invoice_no') or first_page.get('supplier_invoice_no') or ''), 'gstin': str(first_page.get('vendor_gstin') or first_page.get('gstin') or ''), 'dto_memory_ids': dto_memory_ids}
    ...
    def select_best_header(self, group: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def get_header_score(inv):
            ...
            if copy_type == 'original':
                score += 10000
            elif copy_type != 'continuation':
                score += 5000
            essential_fields = ['invoice_no', 'invoice_date', 'vendor_name', 'gstin', 'total_taxable_value', 'total_invoice_value', 'place_of_supply', 'bill_to', 'bill_from']
    ...
    def deduplicate_items(self, items: List[Dict[str, Any]], invoice_no: str=None, group_id: str=None) -> List[Dict[str, Any]]:
        ...
        for itm in items:
            ...
            if key not in seen_keys:
                ...
                logger.info(f"[CLEANUP_DECISION] description='{itm.get('description') or itm.get('item_name')}' reason='cross-page duplicate item' invoice_no={invoice_no} group_id={group_id}")
        non_roundoff_items = []
        for itm in unique_items:
            ...
            if desc in {'rounded off', 'round off', 'rounding adjustment', 'rounding off', 'round_off', 'adjustment'}:
                ...
                logger.info(f"[CLEANUP_DECISION] description='{itm.get('description') or itm.get('item_name')}' reason='rounded off or rounding adjustment item' invoice_no={invoice_no} group_id={group_id}")
                continue
            ...
            if desc in {'total', 'subtotal', 'sub-total', 'summary', 'carried forward', 'brought forward'}:
                ...
                if other_items:
                    ...
                    if taxable_matches or qty_matches:
                        logger.info(f"[ITEM_CLEANUP_GUARD] Dropping generic summary item '{desc}' itm_taxable={itm_taxable} sum_other_taxable={sum_taxable} itm_qty={itm_qty} sum_other_qty={sum_qty}")
                        logger.info(f"[CLEANUP_DECISION] description='{itm.get('description') or itm.get('item_name')}' reason='generic summary row matching mathematical sum of other items' invoice_no={invoice_no} group_id={group_id}")
                        continue
    ...
    def merge_group(self, group: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_b is not None:
                try:
                    ...
                    pass
            pre_merge_info = {'invoice_no': str(page_a.get('invoice_no') or page_a.get('supplier_invoice_no') or ''), 'page_a_items': page_a.get('items', []), 'page_b_items': page_b.get('items', []) if page_b else [], 'page_a_has_final_total': page_a_has_final_total, 'page_b_has_final_total': page_b_has_final_total, 'merge_reason': 'Relational multi-page continuation merge' if page_b else 'Single-page pass-through', 'dto_memory_ids': [str(id(p)) for p in sorted_group]}
        ...
        for inv in sorted_group:
            ...
            for itm in page_items:
                if '_page_no' not in itm:
                    itm['_page_no'] = page_no
                if '_copy_type' not in itm:
                    itm['_copy_type'] = inv.get('_copy_type', 'page_result')
        ...
        if item_count_before > 0 and item_count_after == 0:
            ...
            logger.critical(f"[ITEM_LOSS_DETECTED] invoice_no={merged_invoice.get('invoice_no')} group_id={group_id} pages_in_group={pages_in_group} item_count_before={item_count_before} item_count_after={item_count_after} exact_cleanup_rule_responsible='{rule_responsible}'")
        try:
