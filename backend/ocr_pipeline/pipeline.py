...
from .normalize import lossless_preserve, is_empty, get_canonical_export_record, get_ui_payload, normalize_amount
import django
...
from accounting.models_voucher_purchase import VoucherPurchaseSupplierDetails, VoucherPurchaseSupplyINRDetails, VoucherPurchaseDueDetails
...
def trace_item_checkpoint(record_id, invoice_no, page_number, stage, item_count, item_status, snapshot_item_count=None):
    ...
    logger.info(f'[{stage}] record_id={record_id} invoice_no={invoice_no} page_number={page_number} item_count={item_count} item_status={item_status} snapshot_item_count={snapshot_item_count}')
    redis_key = f'item_trace:{record_id}'
    stages_order = ['ITEM_TRACE_EXTRACTED', 'ITEM_TRACE_AFTER_VALIDATION', 'ITEM_TRACE_AFTER_ASSEMBLY', 'ITEM_TRACE_BEFORE_SNAPSHOT', 'ITEM_TRACE_AFTER_SNAPSHOT', 'ITEM_TRACE_AFTER_HYDRATION']
    try:
    ...
    if previous_stage and previous_item_count > 0 and (item_count == 0):
        logger.critical(f'[ITEM_LOSS_DETECTED] record_id={record_id} invoice_no={invoice_no} previous_stage={previous_stage} current_stage={stage} previous_item_count={previous_item_count} current_item_count={item_count}')
...
def resolve_identity(page):
    ...
    identity = {'invoice_no': final_inv, 'gstin': final_gstin, 'vendor_name': final_vendor, 'invoice_total': final_total, 'items': final_items or []}
    logger.info(f"[IDENTITY_FINAL] inv='{identity['invoice_no']}' gstin='{identity['gstin']}' vendor='{identity['vendor_name']}' total={identity['invoice_total']}")
    return identity
_POISON_GSTIN_VALUES = frozenset(['’', '‘', '—', '–', '“', '”', '…', 'â', '\x93', '\x94', '\x80', '\x99', '\x96', '\x97', '�', '’', '‘', '‚', '‛', '′', '″', '`', '´', 'ΓÇÖ', 'ΓÇ', 'ÇÖ', '—', '—', '–', '—', '’'])
_MIN_PAYLOAD_BYTES = 200
...
def validate_dedup_source(record: InvoiceTempOCR) -> Dict[str, Any]:
    ...
    elif any((c in gstin_stripped for c in ['�', 'â', '\x93', '\x94', '\x80', '\x99'])):
    ...
    bill_from = (canonical.get('bill_from') or canonical.get('bill_address_from') or data.get('bill_from') or data.get('vendor_address') or (data.get('sections') or {}).get('supplier_details', {}).get('bill_from') or '').strip()
    ...
    meaningful_keys = [k for k in data.keys() if not k.startswith('_') and k not in ('total_pages', '_forensics')]
    ...
    UNICODE_POISON_PATTERNS = ['ΓÇÖ', 'ΓÇ', 'ÇÖ', '�']
...
def sync_record_flattened_fields(record: InvoiceTempOCR, data: Dict[str, Any], commit: bool=True):
    ...
    canonical = get_canonical_export_record(data, tenant_id=record.tenant_id)
    valid_fields = {f.name for f in record._meta.get_fields() if getattr(f, 'concrete', False)}
    mapping = {'supplier_invoice_no': str(canonical.get('supplier_invoice_no') or canonical.get('invoice_no') or '')[:100], 'gstin': str(canonical.get('gstin') or '')[:50], 'branch': str(canonical.get('branch') or '')[:255], 'irn': str(canonical.get('irn') or '')[:255], 'ack_no': str(canonical.get('ack_no') or '')[:255], 'ack_date': str(canonical.get('ack_date') or '')[:255], 'vendor_confidence': data.get('vendor_confidence'), 'gstin_confidence': data.get('gstin_confidence'), 'invoice_number_confidence': data.get('invoice_number_confidence')}
    update_fields = []
    ...
    try:
        ...
        if record.extracted_data and (not is_rescan):
            ...
            if dedup_check['valid']:
                logger.info(f"[DEDUP_SOURCE_VALID] record_id={record.id} payload_bytes={dedup_check['details'].get('payload_bytes')} items={dedup_check['details'].get('items_count')} inv_no='{dedup_check['details'].get('invoice_no')}'")
                ...
                logger.error(f"[DEDUP_SOURCE_INVALID] record_id={record.id} source_record='{record.id}' failed_rules={failed_rules} invoice_no='{details.get('invoice_no', '')}' gstin='{details.get('gstin', '')}' items={details.get('items_count', 0)} bill_address_from='{details.get('bill_address_from', '')}' payload_bytes={details.get('payload_bytes', 0)} meaningful_keys={details.get('meaningful_keys_count', 0)}")
                logger.warning(f"[DEDUP_REPLAY_BYPASSED] record_id={record.id} reason='Corrupted replay source. Rules failed: {failed_rules}'. Triggering fresh OCR extraction.")
                record.extracted_data = None
                record.status = 'PENDING'
                record.save(update_fields=['extracted_data', 'status'])
                logger.info(f'[DEDUP_REPROCESS_TRIGGERED] record_id={record.id} corrupted_fields={[k for k in details if k.startswith('corrupted_')]} fresh_extraction=True')
                is_reusable = False
        import pypdf
        import io
        if file_path:
        ...
        for p in range(1, total_pages + 1):
            logger.info(f'[PAGE_CREATED] record_id={record.id} page_number={p} expected_total_pages={total_pages} session={record.upload_session_id}')
        with transaction.atomic():
            state, created = SessionFinalizationState.objects.get_or_create(id=str(record.id), defaults={'expected_pages': total_pages, 'total_pages_expected': total_pages, 'completed_pages': 0, 'failed_pages': 0, 'ai_completed_pages': 0, 'snapshot_created': False})
            ...
            if not record.extracted_data or is_rescan:
                in_flight_statuses = [PipelineStatus.EXTRACTING, PipelineStatus.ASSEMBLING, PipelineStatus.FINALIZING, PipelineStatus.FINALIZED]
        ...
        if not record.extracted_data or is_rescan:
            extracted = extract_invoice(client=None, file_bytes=file_bytes, voucher_type=record.voucher_type or 'Purchase', public_ip='0.0.0.0', user_id='system', tenant_id=str(record.tenant_id or 'system'), wait_for_result=wait_for_ai, record_id=record.id, item_id=item_id, upload_session_id=record.upload_session_id, job_id=job_id, file_path=file_path, is_rescan=is_rescan, rescan_history_id=rescan_history_id)
        else:
        ...
        normalized['_forensics'] = {'raw_extraction': copy.deepcopy(extracted) if isinstance(extracted, dict) else extracted, 'normalized_at': datetime.now().isoformat(), 'pipeline_version': '2.1-flattened'}
        ...
        if '_pages' in extracted:
            for p_no, p_data in extracted['_pages'].items():
                ...
                InvoicePageResult.objects.update_or_create(record_id=record.id, page_number=int(p_no), defaults={'session_id': record.upload_session_id or 'sync', 'canonical_payload': p_data})
        ...
        with transaction.atomic():
            ...
            logger.info(f"[PIPELINE_TRACE] stage='ASSEMBLING' session={record.upload_session_id} record={record.id} inv_no='{record.supplier_invoice_no}' gstin='{record.gstin}' status={record.status}")
            record.save(update_fields=['extracted_data', 'status', 'supplier_invoice_no', 'gstin', 'branch', 'ocr_raw_text', 'normalized_invoice_no', 'vendor_confidence', 'gstin_confidence', 'invoice_number_confidence'])
        ...
        return {'data': normalized, 'validation': res}
    except Exception as e:
        err_msg = str(e).upper()
        ...
        return {'data': {}, 'validation': {'status': 'ERROR', 'error': str(e)}}
...
def assemble_multi_page_record(record: InvoiceTempOCR, **kwargs):
    ...
    try:
        ...
        failed_indices = [res['page_number'] for res in db_results if res['is_failed'] or (isinstance(res['canonical_payload'], dict) and (res['canonical_payload'].get('status') in ('OCR_FAILED', 'EXTRACTION_FAILED', 'NEED_MANUAL_REVIEW') or '_integrity_blocked' in res['canonical_payload']))]
        if failed_indices:
        ...
        for p_idx, k in enumerate(sorted(raw_pages.keys(), key=int), start=1):
            ...
            if page_int in failed_indices:
                p = {'status': 'PARTIAL_FAILED', 'validation_warnings': ['page_ocr_failed'], 'error': 'Page failed raw OCR extraction.', 'invoice_no': 'FAILED', 'vendor_name': 'FAILED'}
            else:
                try:
                    ...
                    p = {'status': 'PARTIAL_FAILED', 'validation_warnings': ['page_normalization_failed'], 'error': f'Page normalization failed: {str(e_page)}', 'invoice_no': 'FAILED', 'vendor_name': 'FAILED'}
            p['_page_no'] = p_idx
            p['_physical_page_no'] = page_int
            p['_runtime_task_id'] = page_int
            pages_list.append(p)
        ...
        logger.critical(f'[PRE_GROUPING_FORENSICS] expected={expected_pages} actual={actual_pages} grouping_pages={grouping_page_ids} missing={missing_pages_pre_group}')
        if missing_pages_pre_group:
            ...
            for missing_p in missing_pages_pre_group:
                pages_list.append({'_physical_page_no': missing_p, '_page_no': missing_p, 'status': 'PARTIAL_FAILED', 'validation_warnings': ['missing_or_failed_page'], 'error': 'This page failed extraction completely.', 'invoice_no': 'FAILED', 'vendor_name': 'FAILED'})
        ...
        for group_id, group_list in groups_dict.items():
            ...
            if raw_pages_list:
                ...
                merged_group['_raw_extraction'] = {'header': primary_raw.get('header') or {}, 'items': flat_items, 'pages': raw_pages_list}
            ...
            logger.info(f'[GROUP_FINALIZED] group_key={group_id} pages={merged_group.get('_source_pages', [p.get('_page_no') for p in group_list])} item_count={len(merged_group.get('items', []))}')
        ...
        for idx, inv in enumerate(_loop_exports):
            ...
            try:
                ...
                items = inv_val['items']
                logger.info(f'[ITEM_EXTRACTION_RESULT] session={record.upload_session_id} invoice_no={invoice_no} item_count={len(items)} item_status={inv_val['item_status']} missing_count={len(inv_val['missing_items'])}')
            except Exception as e_val:
                logger.error(f'[ITEM_VALIDATION_ASSEMBLY_FAILED] error={e_val}')
            try:
                trace_item_checkpoint(record_id=str(record.id), invoice_no=invoice_no, page_number=page_src, stage='ITEM_TRACE_AFTER_VALIDATION', item_count=len(ui_pay.get('items', [])), item_status=ui_pay.get('item_status'), snapshot_item_count=None)
            ...
            totals_empty = normalize_amount(ui_pay.get('total_taxable_value')) == 0.0 and normalize_amount(ui_pay.get('total_invoice_value')) == 0.0 and (normalize_amount(ui_pay.get('total_igst')) == 0.0) and (normalize_amount(ui_pay.get('total_cgst')) == 0.0) and (normalize_amount(ui_pay.get('total_sgst')) == 0.0)
            ...
            if not ui_pay.get('gstin'):
                missing_fields.append('gstin')
            if totals_empty:
                missing_fields.append('totals')
            if not items:
                missing_fields.append('items')
            score_components = [vendor_score * 20, invoice_no_score * 20, gstin_score * 20, totals_score * 20, (1.0 if item_count > 0 else 0.0) * 20]
            ...
            is_completely_empty = len(ocr_text) < 15 and (not is_meaningful(invoice_no)) and (not is_meaningful(vendor_name)) and (not is_meaningful(ui_pay.get('gstin'))) and (not items) and totals_empty
            ...
            ui_pay['_lineage'] = {'source_page': ui_pay.get('_page_no'), 'ocr_confidence': confidence_score, 'merge_stage': 'assemble_multi_page_record', 'normalization_stage': 'get_ui_payload', 'reconciliation_applied': True}
            ...
            logger.info(f'[FINAL_CANONICAL_DTO]\ninvoice_no={invoice_no}\nitem_count={len(items)}\npages={inv.get('_source_pages', [inv.get('_page_no')])}')
        ...
        try:
            ...
            for _dv_inv in final_invoices:
                try:
                    _dv_report = run_document_validation(invoice=_dv_inv, tenant_id=str(record.tenant_id), record_id=str(record.id))
                    _dv_inv['_doc_validation'] = _dv_report.to_dict()
                except Exception as _dv_inv_err:
                    logger.error(f"[DOC_VAL_INVOICE_ERROR] record={record.id} inv='{_dv_inv.get('invoice_no')}' error={_dv_inv_err}")
        except Exception as _dv_err:
            logger.error(f'[DOC_VAL_IMPORT_ERROR] record={record.id} error={_dv_err}')
        for ui_pay in final_invoices:
            try:
                trace_item_checkpoint(record_id=str(record.id), invoice_no=ui_pay.get('invoice_no') or 'N/A', page_number=ui_pay.get('_page_no'), stage='ITEM_TRACE_AFTER_ASSEMBLY', item_count=len(ui_pay.get('items', [])), item_status=ui_pay.get('item_status'), snapshot_item_count=None)
            except Exception as trace_err:
        ...
        if export_rows_count > 0:
            ...
            log_pipeline_event(record.id, PipelineStatus.FINALIZING, session_id=record.upload_session_id, job_id=kwargs.get('job_id'))
            return {'status': 'SUCCESS_EMPTY_EXPORT', 'invoice_count': 0}
        for idx, inv_ui in enumerate(final_invoices):
            logger.critical('[FORENSIC_ITEMS_STRUCTURE]\n%s', json.dumps(inv_ui.get('items'), indent=2, default=str))
            logger.critical('[FORENSIC_ITEMS_LIFECYCLE] [BEFORE_SNAPSHOT_FREEZE] record_id=%s invoice_no=%s item_count=%d item_status=%s payload_keys=%s', inv_ui.get('id'), inv_ui.get('invoice_no'), len(inv_ui.get('items', [])), inv_ui.get('item_status'), list(inv_ui.keys()))
            inv_items = inv_ui.get('items') or []
            logger.critical('[CANONICAL_ITEM_CHECKPOINT] record_id=%s item_count=%d validated_item_count=%d item_status=%s payload_keys=%s', inv_ui.get('id') or record.id, len(inv_items), len(inv_items), inv_ui.get('item_status'), list(inv_ui.keys()))
            if len(inv_items) > 0 and inv_ui.get('item_status') is None:
                raise CriticalPipelineError(f'Item status lost despite extracted items before snapshot freeze: record_id={inv_ui.get('id') or record.id}')
            try:
                trace_item_checkpoint(record_id=str(inv_ui.get('id') or record.id), invoice_no=inv_ui.get('invoice_no') or 'N/A', page_number=inv_ui.get('_page_no'), stage='ITEM_TRACE_BEFORE_SNAPSHOT', item_count=len(inv_items), item_status=inv_ui.get('item_status'), snapshot_item_count=len(inv_items))
            except Exception as trace_err:
                logger.error(f'[TRACE_ERR] ITEM_TRACE_BEFORE_SNAPSHOT: {trace_err}')
        try:
            final_json = {'data': final_invoices, 'metadata': {'total_pages': total_expected, 'assembled_at': datetime.now().isoformat(), 'original_record_id': record.id}}
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f'[DIAGNOSTIC_LOG] existing_siblings count={len(existing_siblings)} details={[{'id': sib.id, 'file_path': getattr(sib, 'file_path', None), 'file_hash': getattr(sib, 'file_hash', None)} for sib in existing_siblings]}')
                siblings = []
                ...
                for idx, inv_ui in enumerate(final_invoices):
                    ...
                    if idx == 0:
                        ...
                        if record.supplier_invoice_no and record.gstin:
                            is_dup = VoucherPurchaseSupplierDetails.objects.filter(supplier_invoice_no__iexact=record.supplier_invoice_no, gstin__iexact=record.gstin, branch__iexact=record.branch, tenant_id=record.tenant_id).exists()
                        if is_dup:
                            record.validation_status = 'DUPLICATE'
                        ...
                        if matched_sib:
                            ...
                            sibling = InvoiceTempOCR(tenant_id=record.tenant_id, upload_session_id=record.upload_session_id, file_path=record.file_path, file_hash=stable_hash, group_id=record.group_id, status=PipelineStatus.FINALIZED, is_primary=is_primary, processed=False, voucher_type=record.voucher_type, upload_type=record.upload_type)
                            sync_record_flattened_fields(sibling, inv_ui, commit=False)
                        ...
                        if sibling.supplier_invoice_no and sibling.gstin:
                            is_dup = VoucherPurchaseSupplierDetails.objects.filter(supplier_invoice_no__iexact=sibling.supplier_invoice_no, gstin__iexact=sibling.gstin, branch__iexact=sibling.branch, tenant_id=sibling.tenant_id).exists()
                        if is_dup:
                ...
                record.extracted_data['validation_revision'] = {'hash': p_hash, 'version': 1, 'timestamp': timezone.now().isoformat(), 'failures': []}
                ...
                for sib in siblings:
                    ...
                    sib.extracted_data['validation_revision'] = {'hash': s_hash, 'version': 1, 'timestamp': timezone.now().isoformat(), 'failures': []}
                if siblings:
                    for sib in siblings:
                        logger.info(f'[PERSISTENCE_ATTEMPT] invoice_no={sib.supplier_invoice_no} group_key=sibling page_count=1 session={record.upload_session_id}')
                    ...
                    if new_siblings:
                        ...
                        created_siblings = list(InvoiceTempOCR.objects.filter(upload_session_id=record.upload_session_id, file_hash__in=new_siblings_hashes))
                    ...
                    referenced_by_pp = set(PendingPurchase.objects.filter(source_scan_row_id__in=sibling_ids).values_list('source_scan_row_id', flat=True))
                    unused_siblings = [sib for sib in existing_siblings if sib.id not in used_existing_ids and sib.id not in referenced_by_pp]
                    protected_siblings = [sib for sib in existing_siblings if sib.id not in used_existing_ids and sib.id in referenced_by_pp]
                    if protected_siblings:
                        logger.warning(f'[STAGING_ROW_CLEANUP_PROTECTED] Preserved unused siblings referenced by PendingPurchase: {[sib.id for sib in protected_siblings]}')
                    logger.info(f'[DIAGNOSTIC_LOG] unused_siblings count={len(unused_siblings)} details={[{'id': sib.id, 'file_path': getattr(sib, 'file_path', None), 'file_hash': getattr(sib, 'file_hash', None)} for sib in unused_siblings]}')
                    ...
                    try:
                        ...
                        for sib in siblings:
                            if sib.id and sib.supplier_invoice_no:
                                updated_count = PendingPurchase.objects.filter(scan_session_id=record.upload_session_id, invoice_number=sib.supplier_invoice_no).update(source_scan_row_id=sib.id, source_document_hash=sib.file_hash)
                                if updated_count > 0:
                                    logger.critical(f'[PENDING_PURCHASE_REASSOCIATED] Updated PP for invoice={sib.supplier_invoice_no} new_source_scan_row_id={sib.id} count={updated_count}')
                    ...
                    for sib in siblings:
                        logger.info(f'[DB_RECORD_CREATED] record_id={sib.id} invoice_no={sib.supplier_invoice_no} group_key=sibling page_count=1')
                ...
                try:
                    snapshot = FinalizedSnapshot.objects.create(session_id=record.upload_session_id, tenant_id=record.tenant_id, job_id=kwargs.get('job_id'), s3_key=s3_key, invoice_count=len(final_invoices), finalized_at=timezone.now())
                    for inv in final_invoices:
                        logger.info(f'[GSTIN_SNAPSHOT] record_id={record.id} gstin={inv.get('gstin')} status={record.status}')
                    ...
                    for inv in final_invoices:
                        try:
                            trace_item_checkpoint(record_id=str(inv.get('id') or record.id), invoice_no=inv.get('invoice_no') or 'N/A', page_number=inv.get('_page_no'), stage='ITEM_TRACE_AFTER_SNAPSHOT', item_count=len(inv.get('items', [])), item_status=inv.get('item_status'), snapshot_item_count=len(inv.get('items', [])))
                        except Exception as trace_err:
                ...
                try:
                    ...
                    raise
                try:
                    logger.info(f'[PERSISTENCE_ATTEMPT] invoice_no={record.supplier_invoice_no} group_key=primary page_count={total_expected} session={record.upload_session_id}')
                    ...
                    record.save()
                    logger.info(f'[DB_RECORD_CREATED] record_id={record.id} invoice_no={record.supplier_invoice_no} group_key=primary page_count={total_expected}')
                ...
                def on_commit_callback():
                    try:
                        ...
                        try:
                            val_query = FinalizedSnapshot.objects.filter(session_id=record.upload_session_id, tenant_id=record.tenant_id)
                        ...
                        if snapshot_count > 0:
                            ...
                            try:
                                from ocr_pipeline.models import log_pipeline_event, PipelineStatus
                                log_pipeline_event(record.id, PipelineStatus.FINALIZED, session_id=record.upload_session_id, job_id=job_id_val)
                            ...
                            orchestrator.update_session_status(str(record.id), 'HYDRATION_READY', progress=100.0, extra_data={'hydration_ready': True})
                        ...
                        logger.info(f'[PIPELINE_PAGE_ACCOUNTING] expected_pages={expected_pages} accounted_pages={accounted_pages} merged_groups={merged_groups} continuations={continuation_pages} partials={partial_pages} orphans={failed_pages_count}')
                        if expected_pages != accounted_pages:
                            logger.error(f"[PIPELINE_PARITY_FAILURE] session_id='{record.upload_session_id}' mismatch detected! expected_pages={expected_pages} accounted_pages={accounted_pages} merged_groups={merged_groups} continuations={continuation_pages} partials={partial_pages} orphans={failed_pages_count}")
        ...
        return {'status': 'SUCCESS', 'snapshot_id': snapshot.id, 'invoice_count': len(final_invoices)}
    except Exception as e:
        ...
        try:
            from ocr_pipeline.models import PipelineStatus
            InvoiceTempOCR.objects.filter(id=record.id).update(status=PipelineStatus.FAILED, validation_status='ERROR')
...
def trigger_next_fanout(record_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(f'[FORENSIC_FANOUT_INVOCATION] timestamp={time.time():.6f} record={record_id} expected={barrier.expected_pages} completed={barrier.completed_pages} failed={barrier.failed_pages} total_pages_completed={barrier.total_pages_completed} db_select_ms={int((t_select_end - t_select_start) * 1000)} isolation_level={isolation_level}')
            ...
            if inflight < MAX_WINDOW:
                ...
                logger.info(f'[FORENSIC_FANOUT_SCHEDULING] record={record_id} inflight={inflight} to_enqueue={to_enqueue} next_start={next_start} remaining={remaining} batch_size={batch_size} redis_slots={redis_slots_str} enqueued_in_redis={enqueued_pages_str} timestamp={time.time():.6f}')
                if batch_size <= 0:
                ...
                for i in range(batch_size):
                    ...
                    try:
                        ...
                        extract_invoice(None, record_id=record.id, file_path=actual_path, wait_for_result=False, tenant_id=record.tenant_id, upload_session_id=record.upload_session_id, job_id=job_id, start_page=page_idx, limit=1, voucher_type=record.voucher_type or 'Purchase')
                        ...
                        try:
                            rec_id_str = str(record_id)
                            page_num_str = str(page_num)
                            already_pushed = orchestrator.redis.get(f'assembly:{rec_id_str}:page:{page_num_str}:enqueued') == 'true' or orchestrator.redis.sismember(f'assembly:{rec_id_str}:enqueued_success_pages', page_num_str)
                        ...
                        if already_pushed:
                            ...
                            SessionFinalizationState.objects.filter(id=str(record_id)).update(total_pages_completed=models.F('total_pages_completed') + 1, updated_at=timezone.now())
                            ...
                            terminalize_page_state(record_id=str(record_id), page_number=page_num, session_id=str(record.upload_session_id), is_failed=True, canonical_payload={'status': 'OCR_FAILED', 'error': f'Enqueue Failed: {e}'}, worker_id='ingestion', queue_source='ingestion_queue')
                            check_and_trigger_assembly(record_id=str(record_id), tenant_id=str(record.tenant_id), session_id=str(record.upload_session_id), correlation_id=f'ingestion_fail_{record_id}_{page_num}', job_id=str(record.upload_session_id), item_id=None)
            else:
...
def force_reconcile_stale_barriers():
    ...
    stuck_records = InvoiceTempOCR.objects.filter(status=PipelineStatus.PROCESSING, created_at__lt=threshold)
...
def run_gst_validation_engine(record: InvoiceTempOCR, user=None):
    logger.critical(f"[GST_TRACE_4] GST engine entered record_id={record.id} validation_status='{record.validation_status}' has_gst_audit_trail={'gst_audit_trail' in (record.extracted_data or {})}")
    ...
    gstin = (canonical.get('canonical_vendor_gstin') or canonical.get('vendor_gstin') or canonical.get('gstin') or record.gstin or data.get('canonical_vendor_gstin') or data.get('vendor_gstin') or data.get('gstin') or '').strip().upper()
    buyer_gstin = (canonical.get('canonical_buyer_gstin') or canonical.get('buyer_gstin') or data.get('canonical_buyer_gstin') or data.get('buyer_gstin') or '').strip().upper()
    ...
    logger.critical(f"[PROBE_3_GST_ENTRY] record_id={record.id} GST engine ENTERED items_count={len(items)} validation_status='{record.validation_status}'")
    try:
        ...
        if extracted_cgst_total == 0.0:
            extracted_cgst_total = extracted_cgst_sum_items
        if extracted_sgst_total == 0.0:
            extracted_sgst_total = extracted_sgst_sum_items
        if extracted_igst_total == 0.0:
            extracted_igst_total = extracted_igst_sum_items
        if extracted_cess_total == 0.0:
            extracted_cess_total = extracted_cess_sum_items
        ...
        logger.critical(f"[PROBE_4_GST_WRITE] record_id={record.id} writing gst_audit_trail validation_status='{validation_status}' is_mismatch={is_mismatch} difference_amount={difference_amount}")
        record.extracted_data['gst_audit_trail'] = {'validation_status': validation_status, 'expected_tax_values': {'cgst': expected_cgst_total, 'sgst': expected_sgst_total, 'igst': expected_igst_total, 'cess': expected_cess_total, 'total_gst': expected_gst_sum, 'total': expected_invoice_total}, 'extracted_tax_values': {'cgst': extracted_cgst_total, 'sgst': extracted_sgst_total, 'igst': extracted_igst_total, 'cess': extracted_cess_total, 'total_gst': extracted_gst_sum, 'total': extracted_invoice_total}, 'difference_amount': difference_amount, 'resolution_choice': gst_resolution, 'resolved_by': resolved_by, 'resolved_at': resolved_at, 'taxable_value': expected_taxable_total, 'gst_rate': gst_rates_str}
        record.save(update_fields=['extracted_data'])
        logger.critical(f"[GST_TRACE_5] GST audit trail written to extracted_data and saved record_id={record.id} validation_status_in_trail='{validation_status}' difference_amount={difference_amount} is_mismatch={is_mismatch}")
        logger.critical(f'[PROBE_5_GST_SAVED] record_id={record.id} gst_audit_trail SAVED to extracted_data in DB. Key now present: {'gst_audit_trail' in record.extracted_data}')
        if is_mismatch and (not gst_resolution):
            record.validation_status = 'GST_MISMATCH'
            record.save(update_fields=['validation_status'])
        elif record.validation_status == 'GST_MISMATCH':
            record.validation_status = 'NEED_TO_SAVE'
            record.save(update_fields=['validation_status'])
    except Exception as gst_err:
        logger.exception(f'[GST_VALIDATION_ERR] record={record.id} invoice={invoice_no} items_count={len(items)} gstin={gstin} error={gst_err}')
...
def validate_and_process(record: InvoiceTempOCR, auto_save: bool=False, **kwargs):
    ...
    """
    logger.critical(f"[GST_TRACE_1] entering validate_and_process record_id={record.id} validation_status='{record.validation_status}' status='{record.status}' processed={record.processed} has_gst_audit_trail={'gst_audit_trail' in (record.extracted_data or {})}")
    ...
    try:
        ...
        from ocr_pipeline.integrity_enforcer import get_dto_hash
        data = record.extracted_data or {}
        is_already_finalized = record.status in {'VOUCHER_CREATED', 'COMPLETED', 'DUPLICATE', 'FAILED', 'ERROR'} or (record.status == 'FINALIZED' and getattr(record, 'processed', False) is True) or getattr(record, 'processed', False) is True or (record.validation_status in {'VOUCHER_CREATED', 'DUPLICATE', 'DUPLICATE_IN_BATCH', 'DUPLICATE_INVOICE'})
        logger.critical(f"[PROBE_1_GATE1_CHECK] record_id={record.id} is_already_finalized={is_already_finalized} validation_status='{record.validation_status}' status='{record.status}' processed={record.processed}")
        logger.critical(f"[GST_TRACE_3] is_already_finalized={is_already_finalized} record_id={record.id} validation_status='{record.validation_status}' status='{record.status}' processed={record.processed} has_gst_audit_trail={'gst_audit_trail' in (record.extracted_data or {})}")
        if is_already_finalized:
            ...
            logger.critical(f"[PROBE_2_GATE1_RETURN] record_id={record.id} RETURNING EARLY -- GST engine will NOT execute. trigger=validation_status='{record.validation_status}'")
    ...
    try:
            ...
            if len(split_invoices) > 1:
                ...
                for i, inv_data in enumerate(split_invoices):
                    ...
                    if i == 0:
                        ...
                        try:
                            child, created = InvoiceTempOCR.objects.update_or_create(file_hash=child_hash, tenant_id=tenant_id, defaults={'upload_session_id': session_id, 'file_path': record.file_path, 'voucher_type': record.voucher_type, 'extracted_data': inv_data, 'supplier_invoice_no': inv_no, 'gstin': gstin, 'branch': branch, 'is_primary': True, 'status': 'EXTRACTED', 'upload_type': record.upload_type})
                            logger.info(f"[SESSION_TRACE_SIBLING] id={child.id} created={created} parent={record.id} session={session_id} inv_no='{inv_no}'")
        ...
        logger.info(f'[VENDOR_VALIDATION_SUCCESS] record={record.id} gstin={gstin} invoice_no={invoice_no} branch={branch_name} vendor_id={record.vendor_id} validation_status={record.validation_status}')
        ...
        if invoice_no and gstin:
            ...
            is_duplicate = VoucherPurchaseSupplierDetails.objects.filter(supplier_invoice_no__iexact=invoice_no, gstin__iexact=canonical_gst, branch__iexact=branch_name, tenant_id=tenant_id).exists()
            norm_inv_no = normalize_invoice_number(invoice_no)
            new_match = False
            if norm_inv_no:
                new_match = VoucherPurchaseSupplierDetails.objects.filter(normalized_invoice_no=norm_inv_no, gstin__iexact=canonical_gst, branch__iexact=branch_name, tenant_id=tenant_id).exists()
            old_match = is_duplicate
            logger.info(f"[DUPLICATE_SHADOW_CHECK] record_id={record.id} invoice_no='{invoice_no}' normalized_invoice_no='{norm_inv_no}' old_match={old_match} new_match={new_match}")
        ...
        if is_duplicate:
            ...
            logger.critical(f"[GST_TRACE_2] duplicate detection result: is_duplicate=True record_id={record.id} inv_no='{invoice_no}' gstin='{gstin}'")
            record.status = 'EXTRACTED'
            record.validation_status = 'DUPLICATE'
            record.save()
            logger.warning(f'[DUPLICATE_STATUS_PERSISTED] id={record.id} validation_status=DUPLICATE')
        logger.critical('[FORENSIC_ITEMS_STRUCTURE]\n%s', json.dumps(items, indent=2, default=str))
        logger.critical('[FORENSIC_ITEMS_LIFECYCLE] [BEFORE_VALIDATION] record_id=%s invoice_no=%s item_count=%d item_status=%s payload_keys=%s', record.id, invoice_no, len(items), (record.extracted_data or {}).get('item_status'), list((record.extracted_data or {}).keys()))
        ...
        logger.critical('[FORENSIC_ITEMS_STRUCTURE]\n%s', json.dumps(inv_val.get('items'), indent=2, default=str))
        logger.critical('[FORENSIC_ITEMS_LIFECYCLE] [AFTER_VALIDATION] record_id=%s invoice_no=%s item_count=%d item_status=%s payload_keys=%s', record.id, invoice_no, len(inv_val.get('items', [])), inv_val.get('item_status'), list(inv_val.keys()))
        raw_item_count = len(items)
        item_status = inv_val.get('item_status')
        if raw_item_count > 0 and item_status is None:
            raise CriticalPipelineError(f'Item status lost despite extracted items: record_id={record.id} raw_item_count={raw_item_count}')
        logger.info(f'[DTO_VALIDATION_STATE] record_id={record.id} vendor_status={('EXISTS' if record.vendor_id else 'NEW')} vendor_id={record.vendor_id} voucher_status={('ALREADY_EXIST' if is_duplicate else 'NEED_TO_SAVE')} item_status={inv_val.get('item_status', 'UNKNOWN')}')
        ...
        PipelineStageTelemetry.record_stage('Validator', {'items_count': len(items)}, {'item_status': inv_val.get('item_status'), 'validation_status': record.validation_status}, val_duration_ms)
        ...
        if record.vendor_id and record.validation_status in {'FOUND', 'READY', 'RESOLVED', 'MATCHED_VENDOR', 'EXISTING_VENDOR', 'NEED_TO_SAVE', 'FINALIZED', 'VOUCHER_CREATED', 'REQUIRES_REVIEW'}:
            ...
            if val_res['status'] == 'EXISTING_VENDOR':
                ...
                if vendor:
                    ...
                    logger.info(f'[VENDOR_VALIDATION_RESULT] gstin={gstin_key} matched_vendor_id={vendor.id} assigned_status=EXISTING_VENDOR record_id={record.id}')
                ...
                vendor = None
                logger.info(f'[VENDOR_VALIDATION_RESULT] gstin={gstin_key} matched_vendor_id=None assigned_status=CREATE_VENDOR record_id={record.id}')
        ...
        ui_row = {'invoice_no': invoice_no, 'invoice_date': canonical.get('invoice_date') or supplier.get('invoice_date') or supply.get('invoice_date') or '', 'vendor_name': _ed_vendor_name, 'vendor_gstin': gstin, 'total_amount': canonical.get('total_amount') or canonical.get('grand_total') or due.get('total_amount') or None}
        is_pending = evaluate_pending_purchase(record, vendor_status_enum, voucher_status_enum, item_status_enum, tenant_id, ui_row=ui_row, auto_save=auto_save)
        if is_pending:
        ...
        try:
            ...
            with transaction.atomic():
                existing_voucher = VoucherPurchaseSupplierDetails.objects.filter(supplier_invoice_no__iexact=invoice_no, gstin__iexact=gstin, branch__iexact=branch_name, tenant_id=tenant_id).first()
                if existing_voucher:
                ...
                for item in items:
                    ...
                    serializer_items.append({'itemCode': item.get('item_code') or '', 'itemName': item.get('description') or '—', 'hsnSac': item.get('hsn_sac') or '', 'qty': qty, 'uom': item.get('uom') or '', 'itemRate': rate, 'taxableValue': tx_val, 'cgst': cgst, 'sgst': sgst, 'igst': igst, 'cess': cess, 'gstRate': to_dec(item.get('gst_rate') or item.get('tax_rate')), 'invoiceValue': inv_val})
                ...
                supply_details = {'purchase_order_no': supply.get('purchase_order_no', ''), 'purchase_ledger': supply.get('purchase_ledger', 'Purchase Account'), 'description': supply.get('description') or f'Auto-validated via OCR Pipeline: {record.file_path}', 'exchange_rate': to_dec(supply.get('exchange_rate', 1.0)), 'items': serializer_items}
                _raw_transit = {'mode': transit.get('mode') or None, 'received_in': transit.get('received_in') or None, 'receipt_date': transit.get('receipt_date') or None, 'receipt_time': transit.get('receipt_time') or None, 'received_quantity': transit.get('received_quantity') or None, 'uqc': transit.get('uqc') or None, 'delivery_type': transit.get('delivery_type') or None, 'self_third_party': transit.get('self_third_party') or None, 'transporter_id': transit.get('transporter_id') or None, 'transporter_name': transit.get('transporter_name') or None, 'vehicle_no': transit.get('vehicle_no') or None, 'lr_gr_consignment': transit.get('lr_gr_consignment') or None}
                ...
                serializer_data = {'date': invoice_date or timezone.now().date(), 'supplier_invoice_no': invoice_no, 'supplier_invoice_date': invoice_date, 'purchase_voucher_series': purchase_series or None, 'purchase_voucher_no': purchase_no or None, 'vendor_id': vendor.id, 'vendor_name': vendor_name or vendor.vendor_name, 'gstin': gstin, 'branch': branch_name or 'Main Branch', 'bill_from': address, 'input_type': 'Interstate' if is_interstate else 'Intrastate', 'invoice_in_foreign_currency': 'Yes' if is_foreign else 'No', 'supply_inr_details': None if is_foreign else supply_details, 'supply_foreign_details': supply_details if is_foreign else None, 'due_details': {'tds_gst': to_dec(due.get('tds_gst', 0.0)), 'tds_it': to_dec(due.get('tds_it', 0.0)), 'advance_paid': to_dec(due.get('advance_paid', 0.0)), 'to_pay': to_dec(due.get('to_pay') or total_inv_val), 'posting_note': due.get('posting_note', ''), 'terms': due.get('terms', due.get('payment_terms', ''))}, 'transit_details': _transit_payload if _has_real_transit else None}
                logger.info(f"[PURCHASE_PAYLOAD_TRACE] record_id={record.id} upload_session_id={record.upload_session_id} supplier_invoice_no='{invoice_no}' vendor_id={(vendor.id if vendor else None)} gstin='{gstin}' branch='{branch_name}' voucher_series='{purchase_series}' voucher_type='purchase' transit_details={serializer_data.get('transit_details')} supply_details={supply_details} due_details={serializer_data.get('due_details')} item_rows={serializer_items}")
                ...
                try:
                    ...
                    if not is_valid_result:
                        logger.error(f"[SERIALIZER_VALIDATION_FAILED] record={record.id} errors={serializer.errors} supplier_invoice_no='{invoice_no}' gstin='{gstin}' branch='{branch_name}' vendor_id={vendor.id}")
                ...
                from accounting.models_voucher_purchase import VoucherPurchaseSupplierDetails as VP_SupplierDetails, VoucherPurchaseSupplyForeignDetails as VP_ForeignDetails, VoucherPurchaseSupplyINRDetails as VP_INRDetails, VoucherPurchaseDueDetails as VP_DueDetails, VoucherPurchaseTransitDetails as VP_TransitDetails, VoucherPurchaseItem as VP_Item
                ...
                logger.info(f'[POST_SAVE_VERIFICATION_STATUS] record={record.id} voucher_id={voucher_main.id} parent_exists={parent_exists} due_exists={due_exists} transit_exists={transit_exists} items_exists={items_exists} inr_exists={inr_exists} foreign_exists={foreign_exists}')
                db_duration_ms = int((time.time() - t_start_db) * 1000) if 't_start_db' in locals() else 0
                from ocr_pipeline.pipeline_telemetry import PipelineStageTelemetry
                PipelineStageTelemetry.record_stage('Database', serializer_data, {'voucher_id': voucher_main.id, 'parent_exists': parent_exists}, db_duration_ms)
                if not parent_exists or not (due_exists and transit_exists and items_exists):
                    logger.error(f'[POST_SAVE_VERIFICATION_FAILED] record={record.id} voucher_id={voucher_main.id} parent={parent_exists} due={due_exists} transit={transit_exists} items={items_exists}')
    ...
    if storage.s3 and storage.bucket:
        ...
        for p in prefixes:
            if clean_key.startswith(p):
                ...
                break
        possible_paths = [os.path.abspath(os.path.join(settings.MEDIA_ROOT, clean_key.replace('/', os.sep))), os.path.abspath(os.path.join(settings.MEDIA_ROOT, 'bulk_pipeline', clean_key.replace('/', os.sep))), os.path.abspath(os.path.join(storage.local_root, clean_key.replace('/', os.sep)))]
        abs_path = None
...
def process_invoice_upload_sync(task: dict):
    ...
    try:
        ...
        result = run_ocr_pipeline(record=record, wait_for_ai=False, job_id=task.get('job_id') or payload.get('job_id'), file_path=abs_file_path, is_rescan=is_rescan, rescan_history_id=rescan_history_id)
