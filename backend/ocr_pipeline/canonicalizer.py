...
class DocumentIdentityCanonicalizer:
    ...
    def canonicalize_invoice_no(raw_val: str) -> Tuple[str, float, str]:
        ...
        raw = re.sub('\\s+', '', str(raw_val).strip()).upper()
        rules_applied = []
        repaired = re.sub('(\\d)S', '\\g<1>5', raw)
        if repaired != raw:
            logger.info(f"[CANONICALIZER_REPAIR_INVOICE_NO] raw='{raw}' repaired='{repaired}' rule='digit_s_to_5'")
            raw = repaired
            rules_applied.append('digit_s_to_5')
        pattern = re.compile('^([A-Z0-9_-]+)([1IL|\\\\/T])(\\d{2,4}-\\d{2,4})$')
        ...
        if match:
            ...
            if sep != '/':
                ...
                logger.info(f"[CANONICALIZER_REPAIR_INVOICE_NO] raw='{raw_val}' canonical='{canonical}' rule='{rule_str}'")
    ...
    def canonicalize_gstin(raw_val: str) -> Tuple[str, float, str]:
        ...
        if len(raw) == 16:
            ...
            if candidates:
                ...
                confidence = 0.97 if best_score == 2 else 0.9
    ...
    def canonicalize_item_name(raw_val: str) -> Tuple[str, float, str]:
        ...
        logger.info(f"[ITEM_CANONICALIZATION] field=item_name raw_value='{raw_val}' canonical_value='{canonical}' confidence={confidence:.3f} repair_rule='{rule}' ops_count={len(ops)}")
    ...
    def canonicalize_hsn(raw_val: str) -> Tuple[str, float, str]:
        ...
        if is_modified:
            ...
            confidence = 1.0
            canonical = raw
        logger.info(f"[ITEM_CANONICALIZATION] field=hsn raw_value='{raw_val}' canonical_value='{canonical}' confidence={confidence:.3f} repair_rule='{rule}'")
        return (canonical, confidence, rule)
    ...
    def canonicalize_vendor_name(raw_val: str) -> Tuple[str, float, str]:
        ...
        if not raw_val:
            return ('', 1.0, 'default_empty')
        raw = str(raw_val).strip()
        canonical = re.sub('\\s+', ' ', raw).strip().upper()
        rules_applied = []
        ...
        logger.info(f"[VENDOR_CANONICALIZATION] raw_vendor_name='{raw}' canonical_vendor_name='{canonical}' confidence={confidence:.3f} repair_rule='{rule_str}'")
        return (canonical, confidence, rule_str)

    @staticmethod
    def canonicalize_buyer_name(raw_val: str) -> Tuple[str, float, str]:
        """
        Safe buyer name normalization.
        Applies space collapse, uppercase, and strips noise prefixes/tags.
        Returns: (canonical_value, confidence, repair_rule)
        """
        if not raw_val:
            return ('', 1.0, 'default_empty')
        ...
        canonical = re.sub(r'^(?:Details\s*of\s*(?:Receiver|Buyer|Customer)(?:\s*\([^)]*\))?\s*:\s*|Billed\s*To\s*:\s*|Buyer\s*:\s*|\d{3,10}\s+)', '', canonical, flags=re.IGNORECASE).strip()
        canonical = re.sub(r'\s*\((?:Billed|Ship(?:ped)?)\s*To\).*', '', canonical, flags=re.IGNORECASE).strip()
        rules_applied = []
        if canonical != raw.upper():
            rules_applied.append('noise_strip')
        if raw != raw.upper():
            rules_applied.append('uppercase')
        rule_str = '+'.join(rules_applied) if rules_applied else 'none'
        confidence = 0.99 if rules_applied else 1.0
        logger.info(f"[BUYER_CANONICALIZATION] raw_buyer_name='{raw}' canonical_buyer_name='{canonical}' confidence={confidence:.3f} repair_rule='{rule_str}'")
        return (canonical, confidence, rule_str)
    ...
    def canonicalize_invoice(cls, invoice: Dict[str, Any], force_canonicalization: bool=False) -> Dict[str, Any]:
        ...
        if enabled and conf_by >= 0.95 and canon_buyer:
            ...
            inv['buyer'] = canon_buyer
        else:
            inv['buyer_name'] = orig_buyer
            inv['customer_name'] = orig_buyer
            inv['buyer'] = orig_buyer
        ...
        PipelineStageTelemetry.record_stage('Canonicalizer', invoice, inv, canon_duration_ms)
        return inv