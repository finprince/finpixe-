...
def hydrate_identity_fields(page: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def resolve(keys: List[str]):
        sources = [(page, 'top-level'), (page.get('header', {}), 'header'), (page.get('_normalized', {}), '_normalized'), (page.get('extracted_data', {}), 'extracted_data')]
        ...
        return (None, None)
    mapping = {'invoice_no': ['invoice_no', 'invoice_number', 'supplier_invoice_no'], 'gstin': ['gstin', 'vendor_gstin', 'supplier_gstin'], 'vendor_name': ['vendor_name', 'supplier_name', 'vendor'], 'invoice_date': ['invoice_date', 'date', 'supplier_invoice_date']}
...
def detect_continuation_markers(text: str) -> List[str]:
    ...
    if not text:
        return markers
    t = text.lower()
    patterns = {'continued_to_page': 'continued\\s+to\\s+page', 'continued': '\\bcontinued\\b', 'carry_forward': 'carry\\s+forward', 'page_2': 'page[\\s-]*2', 'page_2_of_3': 'page[\\s-]*2[\\s-]*of[\\s-]*3', 'item_table_continues': 'item\\s+table\\s+continues', 'subtotal_carried_forward': 'subtotal\\s+carried\\s+forward', 'carried_forward': 'carried\\s+forward', 'brought_forward': 'brought\\s+forward', 'tax_summary': 'tax\\s+summary', 'gst_summary': 'gst\\s+summary'}
...
class ZohoIntegrityEnforcer:
    ...
    def should_merge(self, prev: Dict[str, Any], curr: Dict[str, Any]) -> (bool, str):
        ...
        canonical_match = p_no_c and c_no_c and (p_no_c == c_no_c) and p_gstin_c and c_gstin_c and (p_gstin_c == c_gstin_c) and p_branch_c and c_branch_c and (p_branch_c == c_branch_c) and p_date_c and c_date_c and (p_date_c == c_date_c)
        if canonical_match:
        ...
        logger.info(f'[GROUPING_GSTIN_COMPARE] raw_gstin_a={raw_gstin_a} raw_gstin_b={raw_gstin_b} canonical_gstin_a={canonical_gstin_a} canonical_gstin_b={canonical_gstin_b} exact_match={exact_match} fuzzy_match={fuzzy_match} confidence={ratio:.2f}')
    ...
    def verify(self, final_invoices: List[Dict[str, Any]], original_count: int=0) -> Dict[str, Any]:
        ...
        for idx, inv in enumerate(final_invoices):
            ...
            missing = []
            if not v_name and (not v_inv_no):
                missing.append('vendor_identity')
            if not v_items:
            ...
            inv['validation_revision'] = {'hash': current_hash, 'version': prev_version + 1, 'timestamp': time.time(), 'failures': inv_failures}
        return report
    ...
    def verify_flatten(self, rows: List[Dict[str, Any]], invoices: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(row_inv_nos) != len(source_inv_nos):
            ...
            return {'validation': 'FAIL', 'reason': f'Invoice count mismatch after flattening. Missing: {missing}'}
        ...
        if abs(row_total - source_total) > 10.0:
            return {'validation': 'FAIL', 'reason': f'Value mismatch after flattening: Rows({row_total:.2f}) vs Source({source_total:.2f})'}
