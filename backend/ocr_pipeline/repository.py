...
class StagingRepository:
    ...
    def create_record(self, file_hash, file_name, voucher_type, tenant_id, upload_session_id=None):
        return InvoiceTempOCR.objects.create(file_hash=file_hash, file_path=file_name, voucher_type=voucher_type, tenant_id=str(tenant_id), upload_session_id=upload_session_id, status='QUEUED', created_at=timezone.now(), expires_at=timezone.now() + timezone.timedelta(days=7))
    ...
    def save_results(self, record_id, normalized_json, status='EXTRACTED', vendor_id=None, validation_status=None):
        ...
        InvoiceTempOCR.objects.filter(id=record_id).update(extracted_data=normalized_json, status=status, vendor_id=vendor_id, supplier_invoice_no=supplier.get('supplier_invoice_no'), gstin=supplier.get('gstin'), branch=supplier.get('branch'), validation_status=validation_status, vendor_status='EXISTS' if validation_status in {'FOUND', 'READY', 'RESOLVED'} else 'NEW')