"""
ROOT CAUSE DIAGNOSTIC: UI stuck at "Almost done — building review..."
...
if state:
    ...
    if state.terminal_consistency:
        print("  ✅ terminal_consistency=True => backend GET will return status='FINALIZED'")
    else:
        print(f"  ⚠️  terminal_consistency=False => backend GET returns status='PROCESSING' with progress={backend_pct:.2f}%")
        if backend_pct >= 99.0:
            print("  ❌ STUCK CONDITION: progress_percent hits 99% but FINALIZED not emitted yet!")
else:
    print("  !! NO SessionFinalizationState found for this record !!")
    print("  ❌ STUCK CONDITION: barrier never initialized — GET returns status=PROCESSING, progress=0%")
...
if not snaps.exists():
    print("  ⚠️  No FinalizedSnapshot found — SSE cannot emit FINALIZED even if Redis says FINALIZED!")

# ─── LAYER 4: Redis Orchestrator ─────────────────────────────────────────────
...
try:
    ...
    if redis_status:
        ...
        if redis_status.get('status') in ['COMPLETED', 'FINALIZED']:
            print("  ✅ Redis says FINALIZED")
            if not snaps.exists():
                print("  ❌ But no FinalizedSnapshot => SSE blocks FINALIZED emission (line 1779)")
        else:
            print(f"  ⚠️  Redis status is '{redis_status.get('status')}' — not FINALIZED yet")
    else:
        print("  ⚠️  No Redis state found for this session (may have expired or never set)")
except Exception as e:
    print(f"  !! Redis error: {e}")

# ─── LAYER 5: SSE Simulation ─────────────────────────────────────────────────
...
try:
    ...
    if redis_status and redis_status.get('status') in ['COMPLETED', 'FINALIZED']:
        if snaps.exists() and not is_processing:
            print("  ✅ SSE WOULD EMIT FINALIZED (Redis FINALIZED + snapshot exists + not processing)")
        elif not snaps.exists():
            print("  ❌ SSE BLOCKED: Redis says FINALIZED but FinalizedSnapshot missing!")
        elif is_processing:
            print("  ❌ SSE BLOCKED: Redis says FINALIZED but records still processing!")
    else:
        print("  ⚠️  SSE would NOT emit FINALIZED (Redis not FINALIZED)")
        if not is_processing and snaps.exists():
            print("  ✅ SSE would fall to SNAPSHOT check and emit FINALIZED (fallback path)")
        elif not is_processing and not snaps.exists():
            print("  ❌ SSE FALLBACK ALSO BLOCKED: not processing but no snapshot!")
except Exception as e:
    print(f"  !! Redis check failed: {e}")

# ─── LAYER 6: Backend GET Response Simulation ────────────────────────────────
...
try:
    ...
    if resp_status == 'FINALIZED':
        print("  ✅ GET returns FINALIZED — frontend doFetch would call setScanProgress(100) + setStep('review')")
    elif resp_status == 'PROCESSING' and resp_rows == 0:
        if resp_pct is not None and round(resp_pct) >= 100:
            print("  ❌ STUCK CONDITION CONFIRMED!")
            print("     GET returns PROCESSING + empty data + progress_percent>=100")
            print("     Frontend: pct=100 => 'Almost done — building review...' BUT hydration barrier blocks setStep('review')")
        else:
            print(f"  ⚠️  GET returns PROCESSING + empty data + progress={resp_pct}% — still polling")
    elif resp_status == 'FAILED':
        print(f"  ⚠️  GET returns FAILED + {resp_rows} rows — frontend should setStep('review') with failed data")
except Exception as e:
    import traceback
