...
import io
from .models import InventoryItem, InventoryMasterCategory, InventoryUnit
from core.tenant import get_tenant_from_request
from core.utils import match_headers
logger = logging.getLogger(__name__)
INVENTORY_ITEM_COLUMNS = [{'label': 'Item Code', 'key': 'item_code', 'required': True}, {'label': 'Item Name', 'key': 'item_name', 'required': True}, {'label': 'Description', 'key': 'description', 'required': False}, {'label': 'Category Path', 'key': 'category_path', 'required': False}, {'label': 'UOM', 'key': 'uom', 'required': False}, {'label': 'Alternate UOM', 'key': 'alternate_uom', 'required': False}, {'label': 'Conversion Factor', 'key': 'conversion_factor', 'required': False}, {'label': 'Rate', 'key': 'rate', 'required': False}, {'label': 'Rate Unit', 'key': 'rate_unit', 'required': False}, {'label': 'HSN Code', 'key': 'hsn_code', 'required': False}, {'label': 'GST Rate', 'key': 'gst_rate', 'required': False}, {'label': 'Cess Rate', 'key': 'cess_rate', 'required': False}, {'label': 'Reorder Level', 'key': 'reorder_level', 'required': False}, {'label': 'Is Saleable', 'key': 'is_saleable', 'required': False}]
...
class InventoryItemExcelTemplateDownloadView(APIView):
    ...
    def get(self, request):
        ...
        readme_data = [{'section': 'INVENTORY ITEM DETAILS'}, ('Item Code', 'Mandatory', 'Text', 'Unique identifier for the item (e.g., ITEM001, RM-002).'), ('Item Name', 'Mandatory', 'Text', 'Descriptive name of the inventory item.'), ('Description', 'Optional', 'Text', 'Detailed description of the item.'), ('Category Path', 'Optional', 'Text', "Hierarchy separated by '>' (e.g., Raw Material > Import > HI). Automatically creates category steps if not found."), ('UOM', 'Optional', 'Text', "Unit of Measure (e.g., nos, kg, mtr, ltr). Defaults to 'nos' if empty."), ('Alternate UOM', 'Optional', 'Text', 'Alternate unit of measure for conversion.'), ('Conversion Factor', 'Optional', 'Numeric', 'Conversion factor between main and alternate UOM.'), ('Rate', 'Optional', 'Numeric', 'Standard cost or price rate of the item. Defaults to 0.00.'), ('Rate Unit', 'Optional', 'Text', 'Unit to which the rate applies. Defaults to UOM if empty.'), ('HSN Code', 'Optional', 'Numeric / Text', 'HSN or SAC classification code.'), ('GST Rate', 'Optional', 'Numeric / Percent', 'Tax rate percentage (e.g. 18, 18%, 5, 0).'), ('Cess Rate', 'Optional', 'Numeric', 'Compensation cess rate percentage (e.g. 1.5, 0).'), ('Reorder Level', 'Optional', 'Numeric', 'Threshold quantity to trigger reorder alert.'), ('Is Saleable', 'Optional', 'Yes / No', "Specify 'Yes' or 'No' if this item is for sales. Defaults to 'No'.")]
        current_row = 5
    ...
    def post(self, request):
        ...
        if not excel_file and raw_data:
            ...
            for item in records_to_process:
                ...
                if is_empty(item_code) or is_empty(item_name):
                    results['failed'] += 1
                    results['errors'].append({'message': f'Row {row_idx}: Item Code or Item Name is missing', 'row_data': row_data, 'row_index': row_idx})
                    continue
                item_code_clean = str(item_code).strip()
                if item_code_clean in seen_item_codes:
                    results['failed'] += 1
                    results['errors'].append({'message': f"Row {row_idx}: Duplicate Item Code '{item_code_clean}' in the import list", 'row_data': row_data, 'row_index': row_idx})
                    continue
                ...
                if InventoryItem.objects.filter(tenant_id=tenant_id, item_code=item_code_clean, is_active=True).exists():
                    results['failed'] += 1
                    results['errors'].append({'message': f"Row {row_idx}: Item Code '{item_code_clean}' already exists in the database", 'row_data': row_data, 'row_index': row_idx})
                    continue
                try:
                    with transaction.atomic():
                        ...
                        if not is_empty(cat_path_str):
                            ...
                            if parts:
                                cat_obj, _ = InventoryMasterCategory.objects.get_or_create(tenant_id=tenant_id, category=parts[0], group=parts[1] if len(parts) > 1 else '', subgroup=parts[2] if len(parts) > 2 else '', sub_subgroup=parts[3] if len(parts) > 3 else '')
                        ...
                        def safe_float(val, default=None):
                            ...
                            try:
                                ...
                                return default
                        rate_val = safe_float(row_data.get('Rate'), 0.0)
                        ...
                        inv_item, created = InventoryItem.objects.update_or_create(tenant_id=tenant_id, item_code=str(item_code).strip(), defaults={'item_name': str(item_name).strip(), 'description': str(row_data.get('Description') or '').strip() or None, 'category': cat_obj, 'category_path': cat_obj.full_path if cat_obj else None, 'uom': uom_str, 'alternate_uom': alt_uom_str, 'conversion_factor': conv_val, 'rate': rate_val, 'rate_unit': str(row_data.get('Rate Unit') or uom_str).strip() or 'nos', 'hsn_code': str(row_data.get('HSN Code') or '').strip() or None, 'gst_rate': gst_val, 'cess_rate': cess_val, 'reorder_level': reorder_val, 'is_saleable': is_saleable_bool, 'is_active': True})
                        results['successful_imports'].append({'id': getattr(inv_item, 'id', None), 'item_code': inv_item.item_code, 'item_name': inv_item.item_name, 'row_data': row_data})
                        results['success'] += 1
        ...
        try:
            ...
            if missing_cols:
                ...
                return Response({'summary': {'success': 0, 'failed': 1, 'errors': [{'message': f'Missing required column(s): {col_list}. Please use the correct template.', 'row_data': {}, 'row_index': 'Header'}], 'successful_imports': []}, 'is_preview': True}, status=200)
            records_to_process = []
                    ...
                    with transaction.atomic():
                        ...
                        if not is_empty(cat_path_str):
                            ...
                            if parts:
                                ...
                                cat_obj, _ = InventoryMasterCategory.objects.get_or_create(tenant_id=tenant_id, category=category, group=group, subgroup=subgroup, sub_subgroup=sub_subgroup)
                        ...
                        inv_item = InventoryItem.objects.filter(tenant_id=tenant_id, item_code=str(item_code).strip()).first()
                        if inv_item:
                            ...
                            inv_item = InventoryItem(tenant_id=tenant_id, item_code=str(item_code).strip(), item_name=str(item_name).strip(), description=row_data.get('Description') or None, category=cat_obj, category_path=cat_obj.full_path if cat_obj else None, uom=str(row_data.get('UOM') or 'nos').strip().lower(), alternate_uom=row_data.get('Alternate UOM') or None, conversion_factor=conv_val, rate=rate_val, rate_unit=row_data.get('Rate Unit') or row_data.get('UOM') or 'nos', hsn_code=str(row_data.get('HSN Code') or '').strip() or None, gst_rate=gst_val, cess_rate=cess_val, reorder_level=reorder_val, is_saleable=is_saleable_bool, is_active=True)
                        if not dry_run:
                            inv_item.save()
                        results['successful_imports'].append({'id': None if dry_run else getattr(inv_item, 'id', None), 'item_code': inv_item.item_code, 'item_name': inv_item.item_name, 'row_data': row_data})
                        results['success'] += 1
                        if dry_run:
...
class InventoryItemExcelExportView(APIView):
    ...
    def get(self, request):
        ...
        for row_idx, item in enumerate(items, 2):
            row_data = [item.item_code, item.item_name, item.description or '', item.category_path or (item.category.full_path if item.category else ''), item.uom or 'nos', item.alternate_uom or '', item.conversion_factor if item.conversion_factor is not None else '', item.rate if item.rate is not None else 0.0, item.rate_unit or item.uom or 'nos', item.hsn_code or '', item.gst_rate if item.gst_rate is not None else '', item.cess_rate if item.cess_rate is not None else '', item.reorder_level if item.reorder_level is not None else '', 'Yes' if item.is_saleable else 'No']
            for col_idx, val in enumerate(row_data, 1):
                cell = ws.cell(row=row_idx, column=col_idx, value=val)
                cell.border = border_all
                cell.font = Font(size=10, color='1F2937')
                if col_idx in {7, 8, 11, 12, 13}:
                    if val != '':
                        try:
