...
from .vendorgstdetails_serializers import VendorGSTDetailsSerializer, VendorGSTDetailsCreateSerializer, VendorGSTDetailsUpdateSerializer, VendorGSTDetailsListSerializer
...
class VendorGSTDetailsViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if VendorGSTDetailsDatabase.check_duplicate_gstin(tenant_id, gstin, reference_name):
            ...
            return Response({'error': f'GSTIN "{gstin}" with branch "{reference_name}" is already registered'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            gst_detail = VendorGSTDetailsDatabase.create_gst_detail(tenant_id=tenant_id, gst_data=serializer.validated_data, created_by=username)
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        try:
            updated_gst = VendorGSTDetailsDatabase.update_gst_detail(instance.id, serializer.validated_data, updated_by=username)
            if updated_gst:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if success:
            return Response({'message': 'GST detail deactivated successfully'}, status=status.HTTP_204_NO_CONTENT)
        else:
            return Response({'error': 'GST detail not found'}, status=status.HTTP_404_NOT_FOUND)