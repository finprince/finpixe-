...
from .vendor_serializers import VendorSerializer, VendorCreateSerializer, VendorUpdateSerializer, VendorListSerializer, VendorSummarySerializer, VendorBalanceSerializer, VendorStatisticsSerializer
from .vendorbasicdetail_database import VendorBasicDetailDatabase
from .models import VendorMasterBasicDetail, VendorMasterGSTDetails, VendorMasterTDS, VendorMasterBanking, VendorMasterTerms
...
class VendorViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if vendor_code and VendorDatabase.check_duplicate_vendor_code(tenant_id, vendor_code):
            return Response({'error': f'Vendor code "{vendor_code}" already exists'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            vendor = VendorDatabase.create_vendor(tenant_id=tenant_id, vendor_data=vendor_data, created_by=username)
    ...
    def update(self, request, *args, **kwargs):
        ...
        if email and VendorDatabase.check_duplicate_email(instance.tenant_id, email, exclude_id=instance.id):
            return Response({'error': f'Email "{email}" is already registered'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            ...
            updated_vendor = VendorDatabase.update_vendor(instance.id, update_data, updated_by=username)
            if updated_vendor:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if success:
            return Response({'message': 'Vendor deactivated successfully'}, status=status.HTTP_204_NO_CONTENT)
        else:
            return Response({'error': 'Vendor not found'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def update_balance(self, request, pk=None):
        ...
        try:
            updated_vendor = VendorDatabase.update_vendor_balance(vendor.id, amount, operation)
            if updated_vendor:
                response_serializer = VendorSerializer(updated_vendor)
                return Response(response_serializer.data)
            else:
    ...
    def outstanding(self, request):
        ...
        try:
            ...
            min_balance = 0
        tenant_id = self.get_tenant_id()
        vendors = VendorDatabase.get_vendors_with_outstanding_balance(tenant_id, min_balance)
        serializer = VendorListSerializer(vendors, many=True)
        return Response(serializer.data)
    ...
    def verify(self, request, pk=None):
        ...
        vendor = self.get_object()
        username = self.get_username()
        updated_vendor = VendorDatabase.update_vendor(vendor.id, {'is_verified': True}, updated_by=username)
        if updated_vendor:
            response_serializer = VendorSerializer(updated_vendor)
            return Response(response_serializer.data)
        else:
            return Response({'error': 'Failed to verify vendor'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def activate(self, request, pk=None):
        ...
        vendor = self.get_object()
        username = self.get_username()
        updated_vendor = VendorDatabase.update_vendor(vendor.id, {'is_active': True}, updated_by=username)
        if updated_vendor:
            response_serializer = VendorSerializer(updated_vendor)
            return Response(response_serializer.data)
        else:
            return Response({'error': 'Failed to activate vendor'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def deactivate(self, request, pk=None):
        ...
        vendor = self.get_object()
        username = self.get_username()
        updated_vendor = VendorDatabase.update_vendor(vendor.id, {'is_active': False}, updated_by=username)
        if updated_vendor:
            response_serializer = VendorSerializer(updated_vendor)
            return Response(response_serializer.data)
        else:
            return Response({'error': 'Failed to deactivate vendor'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if gstin:
            ...
            if vendor:
                if vendor.vendor_name.lower() != vendor_name.lower():
                    return Response({'status': 'GSTIN_CONFLICT', 'message': 'GSTIN exists but name differs. Manual verification required.'})
                return Response({'status': 'FOUND', 'matched_by': 'GSTIN', 'vendor_id': vendor.id, 'vendor_name': vendor.vendor_name})
        if vendor_name and state:
            vendor = Vendor.objects.filter(tenant_id=tenant_id, vendor_name__iexact=vendor_name, billing_state__iexact=state).first()
            if vendor:
                return Response({'status': 'FOUND', 'matched_by': 'NAME_STATE', 'vendor_id': vendor.id, 'vendor_name': vendor.vendor_name})
    ...
    def create_from_invoice(self, request):
        ...
        vendor_data = {'vendor_name': vendor_name, 'gstin': gstin if gstin else None, 'billing_address_line1': address, 'billing_state': state, 'notes': 'Created from Invoice Upload', 'vendor_type': 'supplier', 'is_active': True}
        try:
            vendor = VendorDatabase.create_vendor(tenant_id=tenant_id, vendor_data=vendor_data, created_by=username)
            return Response({'status': 'CREATED', 'vendor_id': vendor.id}, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class PurchaseVendorValidateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        print('Purchase Vendor Validation API Hit')
        tenant_id = self.get_tenant_id()
...
class PurchaseVendorResolveConflictView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        gst_record = VendorMasterGSTDetails.objects.filter(tenant_id=tenant_id, gstin__iexact=gstin).select_related('vendor_basic_detail').first()
        ...
        return Response({'success': True, 'status': 'FOUND', 'vendor_id': vendor.id, 'vendor_name': vendor.vendor_name, 'message': f'Resolved using vendor {vendor.vendor_name}'})