...
from .vendorbasicdetail_serializers import VendorBasicDetailSerializer, VendorBasicDetailCreateSerializer, VendorBasicDetailUpdateSerializer, VendorBasicDetailListSerializer, VendorBasicDetailSummarySerializer, VendorBasicDetailStatisticsSerializer
...
class VendorBasicDetailViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if pan_no and VendorBasicDetailDatabase.check_duplicate_pan(tenant_id, pan_no):
            ...
            return Response({'error': f'PAN "{pan_no}" is already registered'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            logger.info(f'Creating vendor basic detail with tenant_id={tenant_id}')
            vendor = VendorBasicDetailDatabase.create_vendor_basic_detail(tenant_id=tenant_id, vendor_data=serializer.validated_data, created_by=username)
    ...
    def update(self, request, *args, **kwargs):
        ...
        if vendor_code and VendorBasicDetailDatabase.check_duplicate_vendor_code(instance.tenant_id, vendor_code, exclude_id=instance.id):
            return Response({'error': f'Vendor code "{vendor_code}" already exists'}, status=status.HTTP_400_BAD_REQUEST)
        email = serializer.validated_data.get('email', instance.email)
        if email and VendorBasicDetailDatabase.check_duplicate_email(instance.tenant_id, email, exclude_id=instance.id):
            return Response({'error': f'Email "{email}" is already registered'}, status=status.HTTP_400_BAD_REQUEST)
        pan_no = serializer.validated_data.get('pan_no', instance.pan_no)
        if pan_no and VendorBasicDetailDatabase.check_duplicate_pan(instance.tenant_id, pan_no, exclude_id=instance.id):
            return Response({'error': f'PAN "{pan_no}" is already registered'}, status=status.HTTP_400_BAD_REQUEST)
        try:
            updated_vendor = VendorBasicDetailDatabase.update_vendor_basic_detail(instance.id, serializer.validated_data, updated_by=username)
            if updated_vendor:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not can_delete:
            ...
            instance.save()
            return Response({'message': 'Vendor has transactions and was deactivated/hidden instead of deleted.'}, status=status.HTTP_200_OK)
        success = VendorBasicDetailDatabase.delete_vendor_basic_detail(instance.id, soft_delete=False)
        if success:
            return Response({'message': 'Vendor deleted successfully from database.'}, status=status.HTTP_200_OK)
        else:
            return Response({'error': 'Vendor not found'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def activate(self, request, pk=None):
        ...
        username = self.get_username()
        updated_vendor = VendorBasicDetailDatabase.update_vendor_basic_detail(vendor.id, {'is_active': True}, updated_by=username)
        if updated_vendor:
            response_serializer = VendorBasicDetailSerializer(updated_vendor)
            return Response(response_serializer.data)
        else:
    ...
    def deactivate(self, request, pk=None):
        ...
        vendor = self.get_object()
        username = self.get_username()
        updated_vendor = VendorBasicDetailDatabase.update_vendor_basic_detail(vendor.id, {'is_active': False}, updated_by=username)
        if updated_vendor:
            response_serializer = VendorBasicDetailSerializer(updated_vendor)
            return Response(response_serializer.data)
        else:
