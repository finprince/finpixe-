...
class RedisOrchestrator:
    ...
    def _connect(self):
        try:
            self.redis = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, db=REDIS_DB, password=REDIS_PASSWORD, decode_responses=True, socket_timeout=5, retry_on_timeout=True)
            self.redis.ping()
    ...
    def get_barrier_state(self, record_id: str, expected_pages: int):
        ...
        logger.info(f'[BARRIER_TERMINAL_PROGRESS] record={record_id} terminal={total_ready} expected={expected_pages} success={completed_count} failed={failed_count} partial={partial_count} continuation={continuation_count}')
        ...
        state = {'completed': completed_count, 'failed': failed_count, 'total': total_ready, 'expected': expected_pages, 'is_ready': is_ready}
        if is_ready:
    ...
    def acquire_ai_slot(self, record_id: str, page_number: int, session_id: str='unknown', tenant_id: str='unknown') -> bool:
        ...
        try:
            ...
            args = [record_id, str(page_number), permit_id, now, expiration, MAX_RECORD_WINDOW, MAX_TENANT_WINDOW]
    ...
    def update_session_status(self, record_id: str, status: str, progress: float=0.0, extra_data: dict=None):
        ...
        def _update():
            ...
            STATE_NORMALIZATION = {'PROCESSING': 'AI_PROCESSING', 'QUEUED': 'INGESTION_STARTED', 'PENDING': 'INGESTION_STARTED', 'INGESTED': 'INGESTION_COMPLETE', 'INGESTING': 'INGESTION_STARTED', 'EXTRACTING': 'AI_PROCESSING', 'ASSEMBLING': 'ASSEMBLY_PENDING', 'DONE': 'COMPLETED', 'EXPORTED': 'FINALIZING', 'FINALIZED': 'HYDRATION_READY', 'MATERIALIZED': 'HYDRATION_READY'}
            ...
            ALLOWED_TRANSITIONS = {None: ['UPLOADED', 'INGESTION_STARTED', 'AI_PROCESSING', 'ASSEMBLY_PENDING', 'FINALIZING'], 'UNKNOWN': ['UPLOADED', 'INGESTION_STARTED', 'AI_PROCESSING', 'ASSEMBLY_PENDING', 'FINALIZING'], 'UPLOADED': ['INGESTION_STARTED', 'FAILED', 'ERROR'], 'INGESTION_STARTED': ['INGESTION_COMPLETE', 'FAILED', 'ERROR'], 'INGESTION_COMPLETE': ['AI_PROCESSING', 'FAILED', 'ERROR'], 'AI_PROCESSING': ['AI_COMPLETE', 'ASSEMBLY_PENDING', 'FAILED', 'ERROR'], 'AI_COMPLETE': ['ASSEMBLY_PENDING', 'ASSEMBLY_COMPLETE', 'FAILED', 'ERROR'], 'ASSEMBLY_PENDING': ['AI_PROCESSING', 'ASSEMBLY_COMPLETE', 'READY_FOR_REVIEW', 'PARTIAL_FAILED', 'FAILED', 'ERROR'], 'ASSEMBLY_COMPLETE': ['FINALIZING', 'READY_FOR_REVIEW', 'PARTIAL_FAILED', 'FAILED', 'ERROR'], 'READY_FOR_REVIEW': ['FINALIZING', 'PARTIAL_FAILED', 'FAILED', 'ERROR'], 'PARTIAL_FAILED': ['FINALIZING', 'FAILED', 'ERROR'], 'FINALIZING': ['MATERIALIZING', 'SNAPSHOTTING', 'HYDRATION_READY', 'FAILED', 'ERROR', 'MATERIALIZATION_PENDING'], 'MATERIALIZATION_PENDING': ['MATERIALIZING', 'FAILED', 'ERROR'], 'MATERIALIZING': ['SNAPSHOTTING', 'HYDRATION_READY', 'FAILED', 'ERROR'], 'SNAPSHOTTING': ['HYDRATION_READY', 'FAILED', 'ERROR'], 'HYDRATION_READY': ['COMPLETED', 'FINALIZED', 'SUCCESS', 'VOUCHER_CREATED'], 'FINALIZED': ['COMPLETED', 'SUCCESS', 'VOUCHER_CREATED'], 'FAILED': ['ERROR'], 'ERROR': ['FAILED'], 'COMPLETED': [], 'SUCCESS': [], 'VOUCHER_CREATED': []}
            ...
            if current_status and current_status != target_status:
                ...
                elif not is_reachable(current_status, target_status):
    ...
    def get_session_status(self, record_id: str):
        ...
        def _get():
            ...
            if data:
                return {'status': data.get('status', 'UNKNOWN'), 'progress': float(data.get('progress', 0.0)), 'extra': {'updated_at': float(data.get('updated_at', 0)), 'hydration_ready': data.get('hydration_ready') == 'true'}}
            return {'status': 'UNKNOWN', 'progress': 0.0, 'extra': {}}
    ...
    def get_redis_metrics(self):
        ...
        def _collect():
            ...
            metrics_data = {'used_memory_mb': info.get('used_memory', 0) / 1024 / 1024, 'ops_per_sec': info.get('instantaneous_ops_per_sec', 0), 'connected_clients': info.get('connected_clients', 0), 'key_count': self.redis.dbsize()}
    ...
    def get_authoritative_session_state(self, session_id: str):
        ...
        def _get_auth():
            ...
            if expected == 0 and completed == 0 and (failed == 0) and (len(record_ids) == 0):
                ...
                return {'session_id': session_id, 'expected_pages': 0, 'completed_pages': 0, 'failed_pages': 0, 'terminal_count': 0, 'barrier_complete': True, 'finalize_complete': True, 'materialization_complete': True, 'snapshot_complete': True, 'ingestion_active': False, 'ai_active': False, 'assembly_active': False, 'finalize_active': False, 'materialization_active': False, 'export_active': False, 'active_workers': 0, 'terminal': True, 'terminal_reason': 'EMPTY_SESSION_TERMINAL'}
            ...
            if all_bootstrapping:
                ...
                failed_bootstrapping_records = InvoiceTempOCR.objects.filter(upload_session_id=session_id, status__in=['FAILED', 'ERROR']).count()
                ...
                logger.info(f'[BOOTSTRAP_VALIDATED] session={session_id} total_records={total_session_records} failed_records={failed_bootstrapping_records}')
                if failed_bootstrapping_records > 0 and failed_bootstrapping_records == total_session_records:
                    hard_ingestion_failed = True
                    expected = total_session_records
                    failed = total_session_records
                    completed = 0
                    logger.info(f'[BARRIER_STATE_INVALID] session={session_id} expected=0 — ALL {total_session_records} records are FAILED (hard ingestion failure). Synthesizing barrier: expected={expected} failed={failed}')
            logger.info(f'[ORCHESTRATOR_CONVERGENCE_CHECK] session={session_id} expected={expected} completed={completed} failed={failed} all_bootstrapping={all_bootstrapping} hard_ingestion_failed={hard_ingestion_failed} redis_all_terminal={redis_all_terminal} redis_hydration_ready={redis_hydration_ready}')
            terminal_count = db_completed + db_failed
            barrier_complete = terminal_count >= expected and expected > 0 or hard_ingestion_failed
            ...
            logger.info(f'[QUEUE_DEPTH_OBSERVABILITY] session={session_id} q_ingest={q_ingest}(v={q_ingest_v},i={q_ingest_i}) q_ai={q_ai}(v={q_ai_v},i={q_ai_i}) q_assembly={q_assembly}(v={q_assembly_v},i={q_assembly_i}) q_finalize={q_finalize}(v={q_finalize_v},i={q_finalize_i}) q_materialize={q_materialize}(v={q_materialize_v},i={q_materialize_i}) q_export={q_export}(v={q_export_v},i={q_export_i}) queues_empty={queues_empty} [METRIC_ONLY — not used as convergence gate]')
            import time
            ...
            if hard_ingestion_failed and queues_empty:
                ...
                logger.info(f'[TERMINAL_RELEASE_GRANTED] session={session_id} reason=HARD_INGESTION_FAILED failed_records={bootstrapping_failed_count}')
            ...
            logger.info(f'[TERMINAL_RELEASE_CHECK] session={session_id} barrier={barrier_complete} snapshot={snapshot_complete} materialization={materialization_complete} queues_empty={queues_empty} active_workers={active_workers} polling_workers={polling_workers} hard_ingestion_failed={hard_ingestion_failed}')
            if not terminal:
                if barrier_complete and snapshot_complete and materialization_complete:
                    ...
                    if not queues_empty:
                        logger.info(f'[QUEUE_ACTIVITY_OBSERVED_AT_TERMINAL] session={session_id} q_ingest={q_ingest} q_ai={q_ai} q_assembly={q_assembly} q_finalize={q_finalize} q_materialize={q_materialize} q_export={q_export} [METRIC_ONLY — convergence already granted]')
                elif not barrier_complete:
            ...
            session_state = {'session_id': session_id, 'expected_pages': expected, 'completed_pages': completed, 'failed_pages': failed, 'terminal_count': terminal_count, 'barrier_complete': barrier_complete, 'finalize_complete': finalize_complete, 'materialization_complete': materialization_complete, 'snapshot_complete': snapshot_complete, 'ingestion_active': q_ingest > 0, 'ai_active': q_ai > 0, 'assembly_active': q_assembly > 0, 'finalize_active': q_finalize > 0, 'materialization_active': q_materialize > 0, 'export_active': q_export > 0, 'active_workers': active_workers, 'terminal': terminal, 'terminal_reason': terminal_reason}
            return session_state
        result = self._safe_exec(_get_auth)
        if result is None:
            return {'session_id': session_id, 'expected_pages': 0, 'completed_pages': 0, 'failed_pages': 0, 'terminal_count': 0, 'barrier_complete': False, 'finalize_complete': False, 'materialization_complete': False, 'snapshot_complete': False, 'ingestion_active': False, 'ai_active': False, 'assembly_active': False, 'finalize_active': False, 'materialization_active': False, 'export_active': False, 'active_workers': 0, 'terminal': False, 'terminal_reason': 'ERROR'}
        return result
