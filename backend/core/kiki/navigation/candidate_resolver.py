...
class NavigationCandidateResolver:
    ...
    EXACT_VERBS = [
        ...
        "switch to", "move to", "view page", "view", "create"
    ]
    ...
    def resolve_candidates(
    ...
    ) -> Tuple[bool, Optional[NavigationNode], List[NavigationNode]]:
        """
        ...
        # Filter by RBAC permissions if provided
        accessible_nodes = cls._filter_by_permissions(all_nodes, user_permissions)

        # 1. Fast-Path Exact Match Lookup (< 1ms, 0 LLM calls)
        for node in accessible_nodes:
            ...
            # Check search_terms exact match
        ...
        # 2. Candidate Retrieval & Capability Scoring for Semantic LLM Reasoning
        ...
        # Fallback to top default workspace pages if no terms scored > 0
    ...
    def _filter_by_permissions(
        ...
        # If superuser or master session
        if user_permissions.get("is_superuser") or user_permissions.get("is_master"):
            return nodes

        filtered = []
        for node in nodes:
            # Check public or core accessible pages
            if node.visibility == "public" or not node.permissions:
                filtered.append(node)
                continue

            # Check if any required permission is present in user_permissions
