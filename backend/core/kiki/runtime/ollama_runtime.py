...
class OllamaRuntime:
    ...
    GREETING_PATTERNS = [
        r"^(hi|hello|hey|greetings|good morning|good afternoon|good evening|thanks|thank you|bye|goodbye)\b"
    ]
    ...
    def _call_llm(self, prompt: str, system_prompt: Optional[str] = None, json_format: bool = False) -> str:
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "temperature": KikiSettings.LLM_TEMPERATURE,
                "top_p": KikiSettings.LLM_TOP_P,
            }
        }
        if system_prompt:
        ...
        try:
            ...
            if response.status_code != 200:
                kiki_logger.error(
                    f"Ollama API HTTP Error: URL={url}, Status={response.status_code}, Model={self.model}, Response={response.text}"
                )
                raise OllamaException(
                    f"Kiki could not communicate with the configured Ollama server.\n"
                    f"Host: {self.host}\n"
                    f"Model: {self.model}\n"
            ...
            raise OllamaException(
                f"Kiki could not connect to Ollama server at {self.host}.\n"
                f"Model: {self.model}\n"
                f"Error: {str(req_err)}"
            ) from req_err
    ...
    def understand_question(self, question: str) -> QuestionUnderstanding:
        ...
        return QuestionUnderstanding(
            intent=parsed.get("intent", "Business Investigation"),
            objective=parsed.get("objective", f"Investigate question: {question}"),
            entities=parsed.get("entities", []),
            timeframe=parsed.get("timeframe", "unknown"),
            scope=parsed.get("scope", "general database")
        )
    ...
    def generate_query_intent(self, context: InvestigationContext) -> QueryIntent:
        ...
        prompt = build_query_intent_prompt(
            question=context.question,
            understanding_dict=understanding_data,
            schemas=schemas_data,
            evidences=evidences_data,
            current_step=context.current_step_number
        )
        ...
        return QueryIntent(
            objective=parsed.get("objective", f"Retrieve data for step {context.current_step_number}"),
            target_tables=target_tbls,
            entity=raw_entity,
            metrics=parsed.get("metrics", ["amount", "total"]),
    ...
    def evaluate_evidence(self, context: InvestigationContext) -> EvaluationResult:
        ...
        prompt = build_evidence_prompt(
            ...
            evidences=evidences_data,
            current_step=context.current_step_number,
            max_steps=KikiSettings.MAX_INVESTIGATION_STEPS
        )
        ...
        return EvaluationResult(
            ...
            next_search_terms=parsed.get("next_search_terms", [])
        )
    ...
    def generate_final_response(self, context: InvestigationContext) -> str:
        ...
        # Clean response to ensure no SQL syntax or technical debug blocks leak to UI
        cleaned_response = re.sub(r'SELECT\s+.*?\s+FROM\s+\w+', '', response_text, flags=re.IGNORECASE)
