...
class SchemaService:
    ...
    def initialize_schema(cls, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if (
            cls._cached_schema is not None
            and not force_refresh
            and (current_time - cls._last_refresh_time) < KikiSettings.SCHEMA_REFRESH_INTERVAL
        ):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                for table_name, col_name, data_type, is_nullable, col_key, comment in col_rows:
                    ...
                    schema_data["tables"][table_name]["columns"].append({
                        "name": col_name,
                        "type": data_type,
                ...
                for table, col, ref_table, ref_col in fk_rows:
                    if table in schema_data["tables"]:
                        schema_data["tables"][table]["foreign_keys"].append({
                            "column": col,
    ...
    def get_table_schema(cls, table_name: str) -> Optional[SchemaMatch]:
        ...
        return SchemaMatch(
            ...
            foreign_keys=t_info["foreign_keys"]
        )
    ...
    def search_schema(cls, search_terms: List[str], max_matches: Optional[int] = None) -> List[SchemaMatch]:
        ...
        if not clean_terms:
            # Fallback to returning top default tables if no terms provided
        ...
        for t_name, t_info in all_tables.items():
            ...
            # High priority score boost for core unified accounting table 'vouchers'
        ...
        # Include 1-hop foreign key related tables
