...
class ForensicBenchmarker:
    ...
    def __init__(self, api_base_url, auth_token, ramp_speed=10):
        ...
        self.results = []
        self.pdf_pool = {'single-page': ['C:\\108\\AI-accounting-0.03 (9)\\backend\\media\\bulk_pipeline\\jobs\\33\\0d96abe7---IMG_20260406_0006.pdf'], 'multi-page': ['C:\\108\\AI-accounting-0.03 (9)\\backend\\media\\bulk_pipeline\\jobs\\337\\b0a24ed3---IMG_20260319_0003.pdf'], 'continuation': ['C:\\108\\AI-accounting-0.03 (9)\\backend\\media\\bulk_pipeline\\jobs\\337\\6d3db0b7---IMG_20260319_0001.pdf'], 'large': ['C:\\108\\AI-accounting-0.03 (9)\\backend\\media\\bulk_pipeline\\jobs\\347\\cd5d5a56---IMG_20260406_0001_Part3.pdf'], 'malformed': ['C:\\108\\AI-accounting-0.03 (9)\\backend\\manage.py']}
    ...
    async def run_tier(self, concurrency_level, chaos=False):
        ...
        async with aiohttp.ClientSession(connector=conn) as session:
            ...
            stats = {'concurrency': concurrency_level, 'total_duration_s': total_duration, 'finalized': finalized_count, 'failed': failed_count, 'timeout': timeout_count, 'upload_p50': statistics.median(latencies) if latencies else 0, 'upload_p95': statistics.quantiles(latencies, n=20)[18] if len(latencies) >= 20 else max(latencies) if latencies else 0, 'e2e_p50': statistics.median(durations) if durations else 0, 'e2e_p95': statistics.quantiles(durations, n=20)[18] if len(durations) >= 20 else max(durations) if durations else 0, 'throughput_ips': finalized_count / total_duration if total_duration > 0 else 0}
