    ...
    while attempt <= MAX_RETRIES:
        try:
            ...
            if redis_client.is_healthy():
                ...
                redis_client.get_client().expire(task_lock_key, 600)
            ...
            res = process_invoice_upload(file_bytes=file_bytes, voucher_type=voucher_type, file_name=file_name, upload_session_id=upload_session_id, tenant_id=tenant_id)
            ...
            return {'item_id': item_id, 'status': 'failed', 'error_message': f'Exhausted {MAX_RETRIES} retries. Last error: {str(e)}'}
        finally:
            db.connections.close_all()
...
def process_bulk_job(job_id: int, voucher_type: str='Purchase'):
    ...
    try:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(worker, iid, job.id, voucher_type, getattr(job, 'upload_session_id', None) or str(job.id), str(job.tenant_id)) for iid in item_ids]
